...
contains
  subroutine compute_basic_coordinate_vars(elem,&
    ...
    real (kind=real_kind), intent(out):: dalpha, dbeta
    real (kind=real_kind), intent(out):: vtx_cart   (nc,nc,2,4)
    real (kind=real_kind), intent(out):: area_sphere(nc,nc)
    real (kind=real_kind), intent(out):: spherecentroid(nc,nc,irecons-1)
    ...
    real (kind=real_kind)    :: acartx(nc+1), acarty(nc+1)
    ...
    do j=1,nc
      do i=1,nc
        centerx = tan(elem%corners(1)%x+(i-0.5D0)*dalpha)
        centery = tan(elem%corners(1)%y+(j-0.5D0)*dbeta)
    ...
    do j=1,nc
      do i=1,nc
        vtx_cart(i,j,1,1) = acartx(i  )
        vtx_cart(i,j,2,1) = acarty(j  )

        vtx_cart(i,j,1,2) = acartx(i+1)
        vtx_cart(i,j,2,2) = acarty(j  )

        vtx_cart(i,j,1,3) = acartx(i+1)
        vtx_cart(i,j,2,3) = acarty(j+1)

        vtx_cart(i,j,1,4) = acartx(i  )
        vtx_cart(i,j,2,4) = acarty(j+1)
      end do
    end do
    ! compute area and centroid for the interior and halo zone of in¯terior elements
    call moment_onsphere(nc,irecons,area_sphere,vtx_cart,.true.,spherecentroid)
  end subroutine compute_basic_coordinate_vars

  subroutine compute_halo_vars(elem,&
       faceno,cubeboundary,nc,nhc,nhe,np,&
       jx_min,jx_max,jy_min,jy_max,&
       flux_orient, ifct, rot_matrix&
       )
    use element_mod, only: element_t
    use control_mod, only : north, south, east, west, neast, nwest, seast, swest
    implicit none
    type (element_t)     , intent(in) :: elem
    integer              , intent(in) :: faceno,nc,nhc,nhe,cubeboundary,np


    integer              , intent(out):: jx_min(3),jx_max(3),jy_min(3),jy_max(3)
    real (kind=real_kind), intent(out):: flux_orient(2 ,1-nhc:nc+nhc,1-nhc:nc+nhc)
  ...
  !-----------------------------------------------------------------------------------!
  subroutine moment_onsphere(nc,irecons,area,vtx_cart,lanalytic,centroid)
    use dimensions_mod, only: ngpc
    implicit none
    integer                                                              , intent(in)  :: nc,irecons
    real (kind=real_kind), dimension(nc,nc)                              , intent(out) :: area
    real (kind=real_kind), dimension(nc,nc,irecons-1), intent(out) :: centroid
    real (kind=real_kind), dimension(nc,nc,2,4)      , intent(in)  :: vtx_cart
    ...
    REAL(KIND=real_kind), dimension(2,num_seg_max,num_area) :: xx, dxx
    integer             , dimension(num_area               ), parameter :: num_seg=2
    REAL(KIND=real_kind), dimension(irecons,num_area):: weights
    real (kind=real_kind), dimension(nc+1) :: x, y
    ...
    x(1:nc) = vtx_cart(1:nc,1   ,1,1)
    y(1:nc) = vtx_cart(1   ,1:nc,2,1)
    x(nc+1) = vtx_cart(  nc,1   ,1,2)
    y(nc+1) = vtx_cart(1   ,nc  ,2,3)
      ...
      if (present(lanalytic)) then
        do j=1,nc
          do i=1,nc
            ...
            centroid(i,j,1) = (I_10(x(i+1),y(j+1)) - I_10(x(i),y(j+1)) + &
                 I_10(x(i),y(j)) - I_10(x(i+1),y(j))) / area(i,j)
            centroid(i,j,2) = (I_01(x(i+1),y(j+1)) - I_01(x(i),y(j+1)) + &
      ...
      if (present(lanalytic)) then
        do j=1,nc
          do i=1,nc
            ...
            centroid(i,j,3) = (I_20(x(i+1),y(j+1)) - I_20(x(i),y(j+1)) + &
                 I_20(x(i),y(j)) - I_20(x(i+1),y(j))) / area(i,j)
            ! TAN(beta)^2 component
            centroid(i,j,4) = (I_02(x(i+1),y(j+1)) - I_02(x(i),y(j+1)) + &
                 I_02(x(i),y(j)) - I_02(x(i+1),y(j))) / area(i,j)
            ! TAN(alpha) TAN(beta) component
            centroid(i,j,5) = (I_11(x(i+1),y(j+1)) - I_11(x(i),y(j+1)) + &
        ...
        do j=1,nc
          do i=1,nc
            ...
            dxx(1,1,1) = x(i+1)-x(i); dxx(2,1,1) = 0.0D0 ;

            xx (1,2,1) = x(i+1)     ; xx (2,2,1) = y(j)  ;
            dxx(1,2,1) = x(i)-x(i+1); dxx(2,2,1) = 0.0D0 ;
            ...
            centroid(i,j,1:5) = weights(2:6,1)/area(i,j)
          end do
        end do
      end if
    case default
...
!     F_02 =-y/SQRT(1.0D0+x*x+y*y)+ASINH(y*COS(alpha))
  ...
  subroutine compute_reconstruct_matrix(nc,nhe,nhc,irecons,dalpha,dbeta,spherecentroid,vtx_cart,&
    ...
    real (kind=real_kind), intent(in) :: dalpha,dbeta
    real (kind=real_kind), dimension(1-nhc:nc+nhc,1-nhc:nc+nhc,irecons-1), intent(in) :: spherecentroid
    real (kind=real_kind), dimension(1-nhc:nc+nhc,1-nhc:nc+nhc,2,4)      , intent(in) :: vtx_cart

    real (kind=real_kind), dimension(1-nhe:nc+nhe,1-nhe:nc+nhe,1:7)      , intent(out):: centroid_stretch
    real (kind=real_kind), dimension(4,1:irecons-1,1-nhe:nc+nhe,1-nhe:nc+nhe), intent(out):: vertex_recons_weights
    real (kind=real_kind), dimension(1-nhe:nc+nhe,1-nhe:nc+nhe,3)        , intent(out):: recons_metrics
    real (kind=real_kind), dimension(1-nhe:nc+nhe,1-nhe:nc+nhe,3)        , intent(out):: recons_metrics_integral

    !
    integer  :: i, j, count, m, n
    real (kind=real_kind) :: coef,tmp,cartx,carty
    !
    ...
    case(3)
      do j= 1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
          ...
          do n = j, j+1
            do m = i, i+1
              cartx = vtx_cart(i,j,1,count); carty = vtx_cart(i,j,2,count);

              vertex_recons_weights(count,1,i,j) = cartx - spherecentroid(i,j,1)
              vertex_recons_weights(count,2,i,j) = carty - spherecentroid(i,j,2)
      ...
      do j= 1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
          ...
          !***************
          !
          coef = 1.0D0/(12.0D0 * dalpha)                   !finite difference coefficient
          coef = coef /( 1.0D0 + spherecentroid(i,j,1)**2) !stretching coefficient

          centroid_stretch(i,j,1) = coef
          !
          !***************
          !*    dfdy     *
          !***************
          !
          coef = 1.0D0/(12.0D0 * dbeta)                    !finite difference coefficient
          coef = coef /( 1.0D0 + spherecentroid(i,j,2)**2) !stretching coefficient

          centroid_stretch(i,j,2) = coef
        end do
      end do
    case(6)
      do j= 1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
          do count=1,4
            ...
            vertex_recons_weights(count,3,i,j) = (spherecentroid(i,j,1)**2 - &
                 spherecentroid(i,j,3))   + &
                 (cartx - spherecentroid(i,j,1))**2
            vertex_recons_weights(count,4,i,j) = (spherecentroid(i,j,2)**2 - &
                 spherecentroid(i,j,4)) + &
                 (carty - spherecentroid(i,j,2))**2

            vertex_recons_weights(count,5,i,j) = (cartx - spherecentroid(i,j,1))*     &
                 (carty - spherecentroid(i,j,2))+     &
                 (spherecentroid(i,j,1) *             &
                 spherecentroid(i,j,2) - &
                 spherecentroid(i,j,5))
          end do
        end do
      end do
      ...
      do j= 1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
          recons_metrics(i,j,1) = spherecentroid(i,j,1)**2 -spherecentroid(i,j,3)
          recons_metrics(i,j,2) = spherecentroid(i,j,2)**2 -spherecentroid(i,j,4)
          recons_metrics(i,j,3) = spherecentroid(i,j,1)*spherecentroid(i,j,2)-&
               spherecentroid(i,j,5)

          recons_metrics_integral(i,j,1) = &
               2.0D0*spherecentroid(i,j,1)**2 -spherecentroid(i,j,3)
          recons_metrics_integral(i,j,2) = &
               2.0D0*spherecentroid(i,j,2)**2 -spherecentroid(i,j,4)
          recons_metrics_integral(i,j,3) = &
               2.0D0*spherecentroid(i,j,1)*spherecentroid(i,j,2)-&
               spherecentroid(i,j,5)
        end do
      end do
      ...
      do j= 1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
          ...
          !*****************
          !
          coef = 1.0D0 / (12.0D0 * dalpha**2)                  !finite difference coefficient
          !
          ! stretching coefficient part 2
          !      recons(3,i,j) = (a * recons(1,i,j)+ recons(3,i,j))*b
          !
          tmp = 0.5D0/((1.0D0 + spherecentroid(i,j,1)**2)**2)

          centroid_stretch(i,j,3) = coef*tmp
          centroid_stretch(i,j,6) = -spherecentroid(i,j,1)/(1.0D0 + spherecentroid(i,j,1)**2)
          ...
          !
          !
          coef = 1.0D0 / (12.0D0 * dbeta**2)                     !finite difference coefficient
          !
          ! stretching coefficient part 2
          !
          !      recons(4,i,j) = (a * recons(1,i,j)+ recons(4,i,j))*b
          !
          tmp =0.5D0/((1.0D0 + spherecentroid(i,j,2)**2)**2)

          centroid_stretch(i,j,4) = coef*tmp
          centroid_stretch(i,j,7) = -spherecentroid(i,j,2)/(1.0D0 + spherecentroid(i,j,2)**2)
          !
          !*****************
          !*    d2fdxdy    *
          !*****************
          !
          !
          coef = 1.0D0 / (4.0D0 * dalpha * dbeta)            !finite difference coefficient
          coef = coef  / ((1.0D0 + spherecentroid(i,j,1)**2) * &
               (1.0D0 + spherecentroid(i,j,2)**2))    !stretching coefficient

          centroid_stretch(i,j,5) = coef
        enddo
      enddo
    case default
      write(*,*) "irecons out of range",irecons
  ...
  subroutine get_high_order_weights_over_areas(x,dx,num_seg,num_seg_max,num_area,weights,ngpc,gsweights, gspts,irecons)
    ...
    REAL(KIND=real_kind), dimension(irecons,num_area), intent(out)   :: weights

    real (kind=real_kind), dimension(ngpc,num_seg_max               ) :: xq,yq        !quadrature points along line segments
    real (kind=real_kind), dimension(ngpc,num_seg_max,irecons) :: F            !potentials
    real (kind=real_kind), dimension(                 irecons) :: weights_area
    ...
    weights(1:irecons,1:num_area) = 0.0D0 !may not be necessary dbgxxx
    do iarea=1,num_area
...
!-----------------------------------------------------------------------------------!
  subroutine create_interpolation_points(elem,&
    ...
    real (kind=real_kind), intent(out) :: halo_interp_weight(1:ns,1-nh:nc+nh,1:nhr,2)
    !
  ...
  ibase = 99999 !dbg
  halo_interp_weight(:,:,:,:) = 9.99E9_real_kind !dbg

  ! element is not on a corner, but shares a cube edge (call of subroutine)
  if(cubeboundary <= 4) then
    gnomxstart(1-nhc)=elem%corners(1)%x-(nhc-0.5D0)*dalpha
    gnomystart(1-nhc)=elem%corners(1)%y-(nhc-0.5D0)*dbeta
    do i=2-nhc,nc+nhc
    ...
    select case (cubeboundary)
      ...
      case(west)
        do halo=1,nhr
!          iref1=ida
          tmpgnom%x=cube_xstart-(halo-0.5D0)*dalpha
          do i=halo-nh,nc+nh-(halo-1) !see fvm_reconstruction to understand these boundaries
            iref1=ida
            tmpgnom%y=gnomystart(i)
            call interpolation_point(nc,ns,nhc,tmpgnom,gnomystart,1,4,1,interp(i,halo,1),&
      ...
      case(east)
        ...
        do halo=1,nhr
          iref1=ida
          tmpgnom%x=cube_xend+(halo-0.5D0)*dalpha
          do i=halo-nh,nc+nh-(halo-1)
            tmpgnom%y=gnomystart(i)
            call interpolation_point(nc,ns,nhc,tmpgnom,gnomystart,1,2,1,interp(i,halo,1),&
                                     ida,ide,iref1,ibase_tmp(i,halo,1))
          end do
        end do
      ...
      case(north)
        ! north zone
        do halo=1,nhr
          tmpgnom%y=cube_yend+(halo-0.5D0)*dbeta
          iref1=ida
          do i=halo-nh,nc+nh-(halo-1)
            ...
            !
            call interpolation_point(nc,ns,nhc,tmpgnom,gnomxstart,1,6,0,interp(i,halo,2),&
      ...
      case(south)
       ...
       do halo=1,nhr
          iref1=ida
          tmpgnom%y=cube_ystart-(halo-0.5D0)*dbeta
          do i=halo-nh,nc+nh-(halo-1)
            tmpgnom%x=gnomxstart(i)
            call interpolation_point(nc,ns,nhc,tmpgnom,gnomxstart,1,5,0,interp(i,halo,2),&
                                     ida,ide,iref1,ibase_tmp(i,halo,2))
          end do
        end do
  ...
  !CORNER TREATMENT
    ...
    gnomxstart(1-nhc)=cube_xstart-(nhc-0.5D0)*dalpha
    gnomxend(nc+nhc)=cube_xend+(nhc-0.5D0)*dalpha
    gnomystart(1-nhc)=cube_ystart-(nhc-0.5D0)*dbeta
    gnomyend(nc+nhc)=cube_yend+(nhc-0.5D0)*dbeta
    do i=2-nhc,nc+nhc
      gnomxstart(i)=gnomxstart(i-1)+dalpha
    ...
    select case (cubeboundary)
      ...
      case(neast)
        ...
        do halo=1,nhr
          ...
          do i=halo-nh,nc+1
            ...
            call interpolation_point(nc,ns,nhc,tmpgnom,gnomyend,1,2,1, interp(i,halo,1),&
                                     ida,ide,iref1,ibase_tmp(i,halo,1))
          end do
       end do
      !CASE NORTH WEST
      case(nwest)
        ...
        do halo=1,2
          ...
          do i=halo-nh,nc+1
            tmpgnom%y=gnomyend(i)
            call interpolation_point(nc,ns,nhc,tmpgnom,gnomyend,1,4,1, interp(i,halo,1),&
                                     ida,ide,iref1,ibase_tmp(i,halo,1))
          end do
       end do
        !THIS CASE SHOULD NOT HAPPEN!
...
subroutine interpolation_point(nc,ns,nhc,gnom,gnom1d,face1,face2,xy,point,ida,ide,iref,ibaseref)
  use coordinate_systems_mod, only : cartesian2D_t
  implicit none
  integer                                , intent(in) :: nc,ns,nhc
  type (cartesian2D_t), intent(in)                    :: gnom
