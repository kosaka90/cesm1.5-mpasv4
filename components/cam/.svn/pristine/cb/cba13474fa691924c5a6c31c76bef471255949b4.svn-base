...
!_______________________________________________________________________
  ...
  hvcoord%nprlev = 0
  do k=1,plev; if(hvcoord%nprlev==0 .and. hvcoord%hybi(k).ne.0.0_r8) hvcoord%nprlev = k - 1; enddo
  ...
  do k = 1,plev
     ...
     amean = ( hvcoord%hyai(k+1) + hvcoord%hyai(k) )*0.5_r8
     bmean = ( hvcoord%hybi(k+1) + hvcoord%hybi(k) )*0.5_r8

     if(amean == 0._r8 .and. hvcoord%hyam(k) == 0._r8) then
        atest = 0.0_r8
     else
        atest = abs( amean - hvcoord%hyam(k) )/ ( 0.5_r8*( abs(amean + hvcoord%hyam(k)) ) )
     endif
     if(bmean == 0._r8 .and. hvcoord%hybm(k) == 0._r8) then
        btest = 0.0_r8
     else
        btest = abs( bmean - hvcoord%hybm(k) )/ ( 0.5_r8*( abs(bmean + hvcoord%hybm(k)) ) )
     endif
