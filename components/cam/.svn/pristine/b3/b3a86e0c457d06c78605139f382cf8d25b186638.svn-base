...
! ^ ifndef CAM
  ...
  real (kind=real_kind), private :: delta  = 1.0e-9_real_kind  ! move tiny bit off center to
...
contains
  ...
  subroutine set_interp_parameter(parm_name, value)
    ...
    if(parm_name .eq. 'itype') then
       ...
       if (value.eq.0) then
           ...
           value_target=value*1.25_real_kind
           power = nint(0.5_real_kind +  log( value_target)/log(2.0_real_kind) )
  ...
  subroutine interpolate_tracers(r, tracers, f)
    ...
    if (.not. interp_tracers_init   ) then
       call abortmp('ERROR: interpolate_tracers() was not initialized')
    endif
...
!  use fvm_reconstruction_mod, only: reconstruction_gradient, recons_val_cart
  ...
  real (kind=real_kind), intent(inout)   :: f(1-nhc:nc+nhc,1-nhc:nc+nhc)
...
!  recons=0.0 ! PCoM
  ...
  x(1:nc) = fvm%vtx_cart(1:nc,1   ,1,1)
  y(1:nc) = fvm%vtx_cart(1   ,1:nc,2,1)
  x(nc+1) = fvm%vtx_cart(  nc,1   ,1,2)
  y(nc+1) = fvm%vtx_cart(1   ,nc  ,2,3)
...
!    flatlon(i)=f(ix,jy)
  ...
  function parametric_coordinates(sphere, corners3D,ref_map_in, corners,u2qmap,facenum) result (ref)
    ...
    real(kind=real_kind)  :: tol_sq = 1.0e-26_real_kind
...
!
  function point_inside_equiangular(elem, sphere, sphere_xyz) result(inside)
    ...
    if ( distance(center,sphere_xyz) > 1.0_real_kind*elem_diam ) return

    tol_inside = 1.0e-10_real_kind*elem_diam**2
...
!
  function point_inside_quad(corners_xyz, sphere_xyz) result(inside)
    ...
    real (kind=real_kind) :: d1,d2, tol_inside = 1.0e-12_real_kind
    ...
    do i=1,4
      ...
      !
       ...
       if (dotprod>0) then
          ...
          enddo
          print *,'test point'
          cart%x = -3.1459_real_kind / 4.0_real_kind
          cart%y = 0
          sphere=projectpoint(cart,5)
        !  print *,'0,0 ->lon,lat: ',sphere%lon,sphere%lat
          cart%x = -3.1459_real_kind / 4.0_real_kind + 0.1_real_kind
          cart%y = 0
          sphere=projectpoint(cart,5)
        !  print *,'+1,0 ->lon,lat: ',sphere%lon,sphere%lat
          cart%x = -3.1459_real_kind/4.0_real_kind + 0.1_real_kind
          cart%y = 0._real_kind + .1_real_kind
          sphere=projectpoint(cart,5)
        !  print *,'+1,+1 ->lon,lat: ',sphere%lon,sphere%lat
          cart%x = -3.1459_real_kind / 4.0_real_kind
          cart%y  = 0._real_kind + 0.1_real_kind
          sphere=projectpoint(cart,5)
        !  print *,'+1,+1 ->lon,lat: ',sphere%lon,sphere%lat
...
!    print *,'WARNING: using GC map'
  ...
  subroutine setup_latlon_interp(elem,interpdata,par)
    ...
    if (gridtype==1 .or. gridtype==3) then
       ...
       if (abs(sum(w(1:plat)) - 2) > 1.0e-8_real_kind) then
       ...
       if (abs(sum(w_staggered(1:plat-1)) - 2) > 1.0e-8_real_kind) then
...
!OMP BARRIER
       ...
       do ie=nets,nete
          ...
          do j=1,nphys
             ...
             !cart3d=spherical_to_cart(fvm(ie)%center_cart(i,j))       ! lat/lon -> 3D
       ...
       do ie=nets,nete
           ...
           do i=1,np
              ...
              if (cube_corner == SEAST .and. ij_corner == SEAST ) then
                 ...
                 if (abs(ref_cart2d%x)>1.05_real_kind .or. abs(ref_cart2d%y)>1.05_real_kind )then
