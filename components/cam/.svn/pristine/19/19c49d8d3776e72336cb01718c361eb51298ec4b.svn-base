...
private
  ...
  real (kind=real_kind), allocatable :: integration_matrix_physgrid(:,:)
...
!  ================================================
  function remap_phys2gll(pin,nphys) result(pout)
    ...
    real(kind=real_kind) :: tol = 1.0e-13_real_kind
...
! grad(psi) dot v:
          ...
          enddo
          enddo
          if (abs(xtmp-div(m,n)) > 3.0e-17_real_kind) then
...
!
  ...
  function subcell_integration(sampled_val, np, intervals, metdet,separate_physics_grid) result(values)
    ...
    if (ltmp) then
        if (.not.ALLOCATED(integration_matrix_physgrid)      .or. &
          ...
          call abortmp( 'FATAL ERROR: allocate_subcell_integration_matrix_physgrid not called')
        end if
  ...
  subroutine limiter_optim_iter_full(ptens,sphweights,minp,maxp,dpmass,kbeg,kend)
    ...
    real (kind=real_kind) :: tol_limiter = 5.0e-14_real_kind
