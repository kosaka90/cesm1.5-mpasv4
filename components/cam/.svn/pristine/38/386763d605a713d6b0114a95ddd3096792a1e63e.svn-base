...
! and if the spherical longitude is in [0,2\pi] or [-\pi,\pi]
  ...
  real(kind=real_kind), public, parameter :: DIST_THRESHOLD= 1.0e-9_real_kind
...
!    end if
    ...
    case (5)
       if (ABS(cart%y) > DIST_THRESHOLD .or. ABS(cart%x) > DIST_THRESHOLD ) then
          ...
          sphere%lon= 0.0_real_kind     ! longitude is meaningless at south pole set to 0.0
       end if
    ...
    case (6)
       if (ABS(cart%y) > DIST_THRESHOLD .or. ABS(cart%x) > DIST_THRESHOLD ) then
          ...
          sphere%lon= 0.0_real_kind     ! longitude is meaningless at north pole set to 0.0
       end if
       sphere%lat=ASIN(one/r)
    case default
    ...
    if (sphere%lon < 0.0_real_kind) then
...
! See unit_face_based_cube_to_unit_sphere for documentation.
  ...
  pure function sphere2cubedsphere (sphere, face_no) result(cart)
    ...
    pi    = DD_PI
    twopi = 2.0_real_kind * pi
    pi2   = pi * 0.5_real_kind
    pi3   = pi * 1.5_real_kind
    pi4   = pi * 0.25_real_kind
