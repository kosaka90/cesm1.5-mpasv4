...
!======================================================================
  ...
  function smallest_diameter_element(element_nodes) result(min_diameter)
    ...
    real(kind=real_kind)             :: x(3), y(3), r(3), d, min_diameter
    ...
    min_diameter = 9999999.0_real_kind
...
!======================================================================
  ...
  subroutine cube_face_element_centroids(centroids, face_numbers, element_nodes)
    ...
    do i=1, p_number_elements
       ...
       centroids(i,:) = SUM(cube_coor,dim=1)/4.0_real_kind
    enddo
...
!======================================================================
  subroutine initialize_space_filling_curve(GridVertex, element_nodes)
    ...
    if (h<.00001_real_kind) then
      call abortmp('initialize_space_filling_curve: Unreasonably small element found. less than .00001')
    end if

    ne = CEILING(0.5_real_kind*DD_PI/(h/2));

    ! find the smallest ne2 which is a power of 2 and ne2>ne
    ne2=2**ceiling( log(real(ne))/log(2._real_kind) )
    ...
    do face = 1,nfaces
       ...
       do i=1,p_number_elements
          if (face_numbers(i) == face ) then
             ...
             i2=nint( (0.5_real_kind + 2.0_real_kind*x/DD_PI)*ne2 + 0.5_real_kind )
             j2=nint( (0.5_real_kind + 2.0_real_kind*y/DD_PI)*ne2 + 0.5_real_kind )
...
!======================================================================
   subroutine MeshCubeTopology(GridEdge, GridVertex)
    ...
    do i=1, p_number_elements
       node_numbers = element_nodes(i,:)
       coordinates = p_node_coordinates(node_numbers,:)
       centroid = SUM(coordinates, dim=1)/4.0_real_kind
