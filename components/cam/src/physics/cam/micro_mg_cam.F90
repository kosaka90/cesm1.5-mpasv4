...
! KSA: for debugging MACv2-SP aerosol for HighResMIP
...
!++CMZ
   call addfld ('MAXREFL',     horiz_only,   'A', 'DBz',      '94 GHz max column radar reflectivity'                              )
!--CMZ
...
!++KSA
   use prescribed_macv2,   only: sp_aop_dNovrN
...
!++CMZ
   real(r8), target :: maxrefl(state%psetcols)    ! max analytic radar reflectivity
!--CMZ
...
!++KSA variables for MACv2-SP aerosol for HighResMIP --------------------
...
!++KSA for MACv2-SP aerosol, Twomey (1st indirect) effect
  if(do_macv2sp) then
       ...
       !this is the last place where ncic_grid is used
       !lambdaac and mu (in physics buffer) are used only by radiation (and MG scheme)
...
!++CMZ
   ...
   do k = 1, pver
     do i = 1, psetcols
       maxrefl(i) = max(arefl(i,k), maxrefl(i))
     end do
   end do
!--CMZ
...
!++CMZ
   call outfld('MAXREFL',     maxrefl,     psetcols, lchnk, avg_subcol_field=use_subcol_microp)
!--CMZ
