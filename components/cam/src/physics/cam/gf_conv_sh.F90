...
!
!org  use module_cu_gf_deep,only:cup_env,cup_env_clev,get_cloud_bc,cup_minimi,  &
  use gf_conv,only:cup_env,cup_env_clev,get_cloud_bc,cup_minimi,  &
  ...
  ! xaa0    = cloud work function with cloud effects (ensemble dependent)
     ...
     flux_tun=fluxtune
      do i=its,itf
        ...
        pre(i)=0._r8
        xmb_out(i)=0._r8
        cap_max_increment(i)=25._r8
        ierrc(i)=" "
        entr_rate(i) = 9.e-5_r8 ! 1.75e-3 ! 1.2e-3 ! .2/50.
      enddo
!
...
!JOE-Georg and Saulo's new idea:
      do i=its,itf
      do k=kts,ktf
          dbyo(i,k)= 0._r8 !hkbo(i)-heso_cup(i,k)
      enddo
      enddo
...
!
      DO i=its,itf
         entr_rate_2d(i,1:ktf)=entr_rate(i)
         IF(ierr(I) == 0)THEN
            start_level(i)=k22(i)
            x_add = xlv*zqexec(i)+cp*ztexec(i)
            call get_cloud_bc(kte,he_cup (i,1:kte),hkb (i),k22(i),x_add)
            ...
            do k=kts,ktf
               frh = 2._r8*min(qo_cup(i,k)/qeso_cup(i,k),1._r8)
               entr_rate_2d(i,k)=entr_rate(i)*(2.3_r8-frh)
...
!
            ktop(i)=1
!            if(k_inv_layers(i,1).gt.0)then                        !WRF3.9 fr
!!               ktop(i)=min(k_inv_layers(i,1),k_inv_layers(i,2))  !WRF3.9 fr
             if(k_inv_layers(i,1).gt.0 .and.   &                   !WRF3.9
                (po_cup(i,kbcon(i))-po_cup(i,k_inv_layers(i,1))).lt.200.)then !WRF3.9
               ktop(i)=k_inv_layers(i,1)
            else
               do k=kbcon(i)+1,ktf
                  if((po_cup(i,kbcon(i))-po_cup(i,k)).gt.200._r8)then  !WRF3.9
                  !if((po_cup(i,kbcon(i))-po_cup(i,k)).gt.100._r8)then !WRF3.9 fr
                    ktop(i)=k
                    exit
                  endif
               enddo
...
! get normalized mass flux profile
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
!           do k=maxloc(zuo(i,1:kte),1),1,-1 ! ktop(i)-1,1,-1
...
!           enddo
           ...
           do k=maxloc(zuo(i,1:ktf),1),ktop(i)
             if(zuo(i,k).lt.1.e-6_r8)then
               ktop(i)=k-1
               exit
             endif
           enddo
      ...
      do 42 i=its,itf
        ...
        IF(ierr(I) /= 0) cycle
         do k=start_level(i)+1,ktop(i)
          ...
          dby(i,k)=max(0._r8,hc(i,k)-hes_cup(i,k))
...
!          dbyo(i,k)=max(0._r8,hco(i,k)-heso_cup(i,k)) !WRF3.9 fr
           dbyo(i,k)=hco(i,k)-heso_cup(i,k)            !WRF3.9
...
!
         do k=start_level(i)+1,ktop(i)
          trash=QESo_cup(I,K)+(1._r8/XLV)*(GAMMAo_cup(i,k) &
                /(1._r8+GAMMAo_cup(i,k)))*DBYo(I,K)
          !- total water liq+vapour
          trash2  = qco(i,k-1) ! +qrco(i,k-1)
          qco (i,k)=   (trash2* ( zuo(i,k-1)-0.5_r8*up_massdetr(i,k-1)) + &
                       up_massentr(i,k-1)*qo(i,k-1))   /            &
                       (zuo(i,k-1)-.5_r8*up_massdetr(i,k-1)+up_massentr(i,k-1))

          !if(qco(i,k)>=trash .and. dbyo(i,k) > 0._r8) then !WRF3.9 fr
          if(qco(i,k)>=trash) then  !WRF3.9
              DZ=Z_cup(i,K)-Z_cup(i,K-1)
              ! cloud liquid water
              qrco(i,k)= (qco(i,k)-trash)/(1._r8+(c0_shal+c1_shal)*dz)
!              qrco(i,k)= (qco(i,k)-trash)/(1.+c0_shal*dz)
         ...
         do k=k22(i)+1,ktop(i)
          ...
          qco(i,k)=qco(i,k)-qrco(i,k) !WRF3.9
         enddo
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
         do k=k22(i),ktop(i)
            ...
            if ( masterproc ) then
               !write(iulog,*)'*********************',i,k,totmas
               write(iulog,*)'sh org bot top ********',i,k,totmas
               write(iulog,*)k22(i),kbcon(i),ktop(i)
            endif
            endif
            dp=100._r8*(po_cup(i,k)-po_cup(i,k+1))
...
!                                 zuo(i,k  )* qrco(i,k  )  )*g/dp
            ...
            dellaq(i,k) =-(zuo(i,k+1)*(qco(i,k+1)-qo_cup(i,k+1) ) -      &
                           ...
                           - C_up - 0.5_r8*(pwo (i,k)+pwo (i,k+1))*g/dp
          enddo
        endif
      enddo
...
!
      ...
      do k=kts,ktf
       do i=its,itf
         ...
         xq (i,k)=max(1.e-16_r8,(dellaq(i,k)+dellaqc(i,k))*mbdt+qo(i,k))
...
!
       do i=its,itf
        ...
        if(ierr(i).eq.0)then
          xmbmax(i)=1.0_r8  
!         xmbmax(i)=100._r8*(p(i,kbcon(i))-p(i,kbcon(i)+1))/(g*dtime)
!
!-stabilization closure
          xkshal=(xaa0(i)-aa1(i))/mbdt
             if(xkshal.le.0._r8 .and. xkshal.gt.-.01_r8*mbdt) &
                           xkshal=-.01_r8*mbdt
             if(xkshal.gt.0._r8 .and. xkshal.lt.1.e-2_r8) &
                           xkshal=1.e-2_r8

          xff_shal(1)=max(0._r8,-(aa1(i)-aa0(i))/(xkshal*dtime))
!
!- closure from Grant (2001)
          xff_shal(2)=.03_r8*zws(i)
!- boundary layer QE closure
          ...
          enddo
          trash=max((hc(i,kbcon(i))-he_cup(i,kbcon(i))),1.e1_r8)
          xff_shal(3)=max(0._r8,blqe/trash)
          xff_shal(3)=min(xmbmax(i),xff_shal(3))
!- average 
          xmb(i)=(xff_shal(1)+xff_shal(2)+xff_shal(3))/3._r8
...
!
          ...
          do k=2,ktop(i)
           ...
           !dlfs(i,k)= dellaqc(i,k)*xmb(i)
           !outqc(i,k)= outqc(i,k)-dlfs(i,k)
