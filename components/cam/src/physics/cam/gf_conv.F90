...
!   real(r8), DIMENSION( pcols ) :: cap_suppress_j
!#endif
  !
  !
      real(r8),    dimension (pcols,1:maxens3) :: xf_ens,pr_ens
  ! outtem = output temp tendency (per s)
  ! outq   = output q tendency (per s)
  ! outqc  = output qc tendency (per s)
  ! pre    = output precip
     real(r8),    dimension (pcols,kts:kte)                              &
        ,intent (inout  )                   ::                           &
  ...
  !
     ...
     real(r8),    dimension (pcols)                                      &
        ,intent (inout  )                   ::                           &
        tau_ecmwf_out,aa1_out
     real(r8),    dimension (pcols,kts:kte)                              &
        ,intent (inout)                   ::                           &
         evap_out,dlf,rprd

      real(r8),    dimension (pcols,1:maxens3) &
         ,intent (inout) :: xf_ens1_out, xf_ens2_out
...
!
     real(r8),    dimension (pcols,1)  ::                         &
        xaa0_ens
     real(r8),    dimension (pcols,1) ::                         &
        edtc
     real(r8),    dimension (pcols,kts:kte,1) ::                 &
...
!
  ...
  ! hkb = moist static energy at originating level
     ...
     real(r8),    dimension (pcols,kts:kte) ::                           &
        ...
        dbyt,xdby,xhc,xzu,            & !add dbyt (WRF3.9)
  ...
  ! edt     = epsilon
     ...
     integer,    dimension (pcols) ::                                &
       kzdown,KDET,kstabi,kstabm,K22x,xland1,        &
       !kzdown,KDET,K22,JMIN,kstabi,kstabm,K22x,xland1,        &
       ktopdby,KBCONx,ierr2,ierr3,KBMAX

     integer,  dimension (pcols), intent(inout) :: ierr
     integer,  dimension (pcols), intent(in) :: csum
     integer                              ::                           &
     ...
     real(r8) entdo,dp,subin,detdo,entup,                &
      detup,subdown,entdoj,entupk,detupk,totmas

     real(r8), dimension (pcols) :: lambau,flux_tun,zws,ztexec,zqexec

     integer :: jprnt,jmini,start_k22
     logical :: keep_going,flg(pcols)
     ...
     real(r8),    dimension (pcols,kts:kte) ::                           &
       up_massentru,up_massdetru,dd_massentru,dd_massdetru !add dd_mass (WRF3.9)
     real(r8) buo_flux,pgcon,pgc,blqe

     real(r8) :: xff_mid(pcols,2)
     integer :: iversion=1
     real(r8) :: denom,h_entr,umean,t_star,dq
     integer, intent(IN) :: DICYCLE
     real(r8),    dimension (pcols) :: aa1_bl,hkbo_bl,tau_bl,tau_ecmwf,wmean
     real(r8), dimension (pcols,kts:kte) :: tn_bl, qo_bl, qeso_bl, heo_bl, heso_bl &
     					 ,qeso_cup_bl,qo_cup_bl, heo_cup_bl,heso_cup_bl&
     					 ,gammao_cup_bl,tn_cup_bl,hco_bl,DBYo_bl
     real(r8), dimension(pcols) :: xf_dicycle
     real(r8), intent(inout), dimension(pcols,10) :: forcing
     integer :: pmin_lev(pcols),start_level(pcols),ktopkeep(pcols) !add ktopkeep (WRF3.9)
     real(r8),    dimension (pcols,kts:kte) :: dtempdz
     integer, dimension (pcols,kts:kte) ::  k_inv_layers

! rainevap from sas
     real(r8) zuh2(40)
     real(r8), dimension (pcols) :: rntot,delqev,delq2,qevap,rn,qcond
     real(r8) :: rain,t1,q1,elocp,evef,el2orc,evfact,evfactl,g_rain,e_dn,c_up
     real(r8) :: pgeoh,dts,fp,fpi,pmin,x_add,beta,beta_u
     real(r8) :: cbeg,cmid,cend,const_a,const_b,const_c

!-- END of registration of variables
!===============================================================================

      flux_tun(:itf)=fluxtune
!      if(imid.eq.1)flux_tun(:itf)=fluxtune+.5
      pmin=150._r8
      if(imid.eq.1)pmin=75._r8
      ktopdby(:itf)=0
      elocp=xlv/cp
...
! here random must be between -1 and 1
       if(nranflag == 1)then
           lambau(:itf)=1.5_r8+rand_mom(:itf)
       endif
! SAS
!     lambau=0._r8  !wrf 3.9
!     pgcon=-.55
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do i=its,itf
         !- buoyancy flux (H+LE)
         buo_flux= (hfx(i)/cp+0.608_r8*t(i,1)*qfx(i)/xlv)/rho(i,1)
         pgeoh = zo(i,2)*g
         !-convective-scale velocity w*
         zws(i) = max(0._r8,flux_tun(i)*0.41_r8*buo_flux*zo(i,2)*g/t(i,1))
         if(zws(i) > TINY(pgeoh)) then
            !-convective-scale velocity w*
            zws(i) = 1.2_r8*zws(i)**.3333_r8
            !- temperature excess
            ztexec(i)     = MAX(flux_tun(i)*hfx(i)/(rho(i,1)*zws(i)*cp),0.0_r8)
            !- moisture  excess
            zqexec(i)     = MAX(flux_tun(i)*qfx(i)/xlv/(rho(i,1)*zws(i)),0._r8)
         endif
         !- zws for shallow convection closure (Grant 2001)
         !- height of the pbl
         zws(i) = max(0._r8,flux_tun(i)*0.41_r8*buo_flux*zo(i,kpbl(i))*g/t(i,kpbl(i)))
         zws(i) = 1.2_r8*zws(i)**.3333_r8
...
!     if(imid.eq.1)cap_maxs=100.
      do i=its,itf
        ...
        xmb_out(i)=0._r8
        cap_max(i)=cap_maxs
        cap_max_increment(i)=20._r8  !25. (WRF3.9 fr1)
        if(imid.eq.1)cap_max_increment(i)=10._r8 !add WRF3.9
!
! for water or ice
!
        xland1(i)=int(xland(i)+.0001_r8) ! 1.
        if(xland(i).gt.1.5_r8 .or. xland(i).lt.0.5_r8)then
            xland1(i)=0
!            if(imid.eq.0)cap_max(i)=cap_maxs-25.
!            if(imid.eq.1)cap_max(i)=cap_maxs-50.
            cap_max_increment(i)=20. !add WRF3.9
        else
            if(ztexec(i).gt.0.)cap_max(i)=cap_max(i)+25. !add WRF3.9
            if(ztexec(i).lt.0.)cap_max(i)=cap_max(i)-25. !add WRF3.9
        endif
        ierrc(i)=" "
...
!      do i=its,itf
!          cap_max(i)=cap_maxs
!          if (abs(cap_suppress_j(i) - 1.0_r8 ) < 0.1_r8 ) then
!             cap_max(i)=cap_maxs+75._r8
!          elseif (abs(cap_suppress_j(i) - 0.0_r8 ) < 0.1_r8 ) then
!             cap_max(i)=10.0_r8
!          endif
!      enddo
!  endif
...
!
      start_level(:itf)=kte
      do i=its,ite
         c1d(i,:)= 0._r8 !c1 ! 0. ! c1 ! max(.003,c1+float(csum(i))*.0001)
         entr_rate(i)=7.e-5_r8 - min(20._r8,float(csum(i))) * 3.e-6_r8
         if(xland1(i) == 0)entr_rate(i)=7.e-5_r8
         if(imid.eq.1)entr_rate(i)=1.e-4_r8
!         if(imid.eq.1)c1d(i,:)=c1
         radius=.2_r8/entr_rate(i)              !WRF3.9
         frh=min(1., 3.14_r8*radius*radius/dx(i)/dx(i))  !WRF3.9
!         radius=.1_r8/entr_rate(i)             !WRF3.9 fr1
!         frh=3.14_r8*radius*radius/dx(i)/dx(i) !WRF3.9 fr1
         if(frh > frh_thresh)then
            frh=frh_thresh
            radius=sqrt(frh*dx(i)*dx(i)/3.14_r8)
            entr_rate(i)=.2_r8/radius
         endif
         sig(i)=(1._r8-frh)**2
      enddo
      sig_thresh = (1._r8-frh_thresh)**2
...
!
      edtmax(:itf)=1._r8
      if(imid.eq.1)edtmax(:itf)=.15_r8
      edtmin(:itf)=.1_r8
      if(imid.eq.1)edtmin(:itf)=.05_r8
!
!--- minimum depth (m), clouds must have
!
      depth_min=1000._r8
      if(imid.eq.1)depth_min=500._r8
!
...
!
      zkbmax=4000._r8
      if(imid.eq.1)zkbmax=2000._r8
!
!--- height(m) above which no downdrafts are allowed to originate
!
      !zcutdown=3000._r8 !WRF3.9 fr1
      zcutdown=4000._r8
!
...
!++JHJ - initialize the out variables
      do i=its,itf
         sig_out(i)=0._r8
            do k=1,maxens3
...
!
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
          ...
          do k=kts+1,ktf
           u_cup(i,k)=.5_r8*(us(i,k-1)+us(i,k))
           v_cup(i,k)=.5_r8*(vs(i,k-1)+vs(i,k))
          enddo
        endif
      enddo
      do i=its,itf
...
!
      ...
      DO i=its,itf
         IF(ierr(I) == 0)THEN
           frh = min(qo_cup(i,kbcon(i))/qeso_cup(i,kbcon(i)),1._r8)
...
!
      if(imid.eq.1)then
      !call get_inversion_layers(ierr,p_cup,t_cup,z_cup,q_cup,qes_cup,k_inv_layers,       & !bug fix?
      call get_inversion_layers(ierr,po_cup,tn_cup,zo_cup,qo_cup,qeso_cup,k_inv_layers,       &
...
!         if(imid.eq.0 .and. pmin_lev(i).lt.kbcon(i)+3)pmin_lev(i)=kbcon(i)+3
            ...
            do k=kts,ktf
               frh = min(qo_cup(i,k)/qeso_cup(i,k),1._r8)
               entr_rate_2d(i,k)=entr_rate(i) *(1.3_r8-frh)
            enddo
            if(imid.eq.1)then
                if(k_inv_layers(i,2).gt.0 .and.   &        !WRF3.9
                  (po_cup(i,k22(i))-po_cup(i,k_inv_layers(i,2))).lt.500._r8)then  !WRF3.9
                 ktop(i)=min(kstabi(i),k_inv_layers(i,2))  !WRF3.9

!               if(k_inv_layers(i,2) > 0 )then !WRF3.9 fr1
!                 ktop(i)=k_inv_layers(i,2)    !WRF3.9 fr1

                 ktopdby(i)=ktop(i)
               else
                 do k=kbcon(i)+1,ktf       !WRF3.9
                  if((po_cup(i,k22(i))-po_cup(i,k)).gt.500._r8)then !WRF3.9
                    ktop(i)=k              !WRF3.9
                    ktopdby(i)=ktop(i)     !WRF3.9
                    exit                   !WRF3.9
                  endif                    !WRF3.9
                 enddo                     !WRF3.9
!                  ktop(i)=kstabi(i)+1     !WRF3.9 fr1
!                  ktopdby(i)=kstabi(i)+1  !WRF3.9 fr1
               endif ! k_inv_lay
            endif
...
!       if(ierr(i) /= 0) cycle  !WRF3.9 fr1

       ktopkeep(i)=0            !WRF3.9
       dbyt(i,:)=0._r8          !WRF3.9
       if(ierr(i) /= 0) cycle   !WRF3.9        
       ktopkeep(i)=ktop(i)      !WRF3.9

       DO k=start_level(i) +1,ktop(i)  !mass cons option

	  denom=zuo(i,k-1)-.5_r8*up_massdetro(i,k-1)+up_massentro(i,k-1)
          if(denom.lt.1.e-8_r8)then
           ierr(i)=51
           exit
          endif

          hc(i,k)=(hc(i,k-1)*zu(i,k-1)-.5_r8*up_massdetr(i,k-1)*hc(i,k-1)+ &
                                          up_massentr(i,k-1)*he(i,k-1))   /            &
                            (zu(i,k-1)-.5_r8*up_massdetr(i,k-1)+up_massentr(i,k-1))
          uc(i,k)=(uc(i,k-1)*zu(i,k-1)-.5_r8*up_massdetru(i,k-1)*uc(i,k-1)+ &
                                          up_massentru(i,k-1)*us(i,k-1) &
                            -pgcon*.5_r8*(zu(i,k)+zu(i,k-1))*(u_cup(i,k)-u_cup(i,k-1))) /     &
                           (zu(i,k-1)-.5_r8*up_massdetru(i,k-1)+up_massentru(i,k-1))
          vc(i,k)=(vc(i,k-1)*zu(i,k-1)-.5_r8*up_massdetru(i,k-1)*vc(i,k-1)+                &
                                          up_massentru(i,k-1)*vs(i,k-1)      &
                         -pgcon*.5_r8*(zu(i,k)+zu(i,k-1))*(v_cup(i,k)-v_cup(i,k-1))) /    &
                         (zu(i,k-1)-.5_r8*up_massdetru(i,k-1)+up_massentru(i,k-1))
          dby(i,k)=hc(i,k)-hes_cup(i,k)
          hco(i,k)=(hco(i,k-1)*zuo(i,k-1)-.5_r8*up_massdetro(i,k-1)*hco(i,k-1)+ &
                                             up_massentro(i,k-1)*heo(i,k-1))   /            &
                         (zuo(i,k-1)-.5_r8*up_massdetro(i,k-1)+up_massentro(i,k-1))
          dbyo(i,k)=hco(i,k)-heso_cup(i,k)

          DZ=Zo_cup(i,K+1)-Zo_cup(i,K)       !WRF3.9
          dbyt(i,k)=dbyt(i,k-1)+dbyo(i,k)*dz !WRF3.9
       ENDDO
...
41    continue
      DO i=its,itf
       ...
       do k=ktop(i)+1,ktf
           ...
           zuo(i,k)=0._r8
           cd(i,k)=0._r8
           entr_rate_2d(i,k)=0._r8
           up_massentr(i,k)=0._r8
           up_massdetr(i,k)=0._r8
           up_massentro(i,k)=0._r8
           up_massdetro(i,k)=0._r8
       enddo
      ENDDO
!
      DO i=its,itf
      ...
      DO 37 i=its,itf
         kzdown(i)=0
         if(ierr(i).eq.0)then
            zktop=(zo_cup(i,ktop(i))-z1(i))*.6_r8
            if(imid.eq.1)zktop=(zo_cup(i,ktop(i))-z1(i))*.4_r8
...
!
      ...
      do i=its,itf
       zdo(i,k)=0._r8
       cdd(i,k)=0._r8
       dd_massentro(i,k)=0._r8
       dd_massdetro(i,k)=0._r8
       dd_massentru(i,k)=0._r8 !WRF3.9
       dd_massdetru(i,k)=0._r8 !WRF3.9
      ...
      enddo
      do i=its,itf
        beta=max(.02_r8,.05_r8-float(csum(i))*.0015_r8)  !.02
!        beta=max(.05,.08-float(csum(i))*.0015)  !.02
        if(imid.eq.0 .and. xland1(i) == 0)then
!             beta=.01
              edtmax(i)=max(0.1_r8,.4_r8-float(csum(i))*.015_r8) !.3)
        endif
        if(imid.eq.1)beta=.02_r8
        bud(i)=0._r8
        IF(ierr(I).eq.0)then
        cdd(i,1:jmin(i))=1.e-9_r8
        cdd(i,jmin(i))=0._r8
        dd_massdetro(i,:)=0._r8
        dd_massentro(i,:)=0._r8
           !call get_zu_zd_pdf_fim(rand_vmas(i),0._r8,ipr,xland1(i),zuh2,"DOWN",ierr(i),kdet(i),jmin(i),zdo(i,:),kts,kte,ktf,beta,kpbl(i),csum(i),pmin_lev(i))  !WRF3.9 fr1
        !check!!
        call get_zu_zd_pdf_fim(0,po_cup(i,:),rand_vmas(i),0._r8,ipr,xland1(i),zuh2,"DOWN",ierr(i),kdet(i),jmin(i),zdo(i,:),kts,kte,ktf,beta,kpbl(i),csum(i),pmin_lev(i))   !WRF3.9 
        if(zdo(i,jmin(i)) .lt.1.e-8_r8)then
          zdo(i,jmin(i))=0._r8
          jmin(i)=jmin(i)-1
          if(zdo(i,jmin(i)) .lt.1.e-8_r8)then
             ierr(i)=876
        ...
        do ki=jmin(i)  ,maxloc(zdo(i,:),1),-1
          ...
          if(dd_massentro(i,ki).lt.0._r8)then
             dd_massentro(i,ki)=0._r8
             dd_massdetro(i,ki)=zdo(i,ki+1)-zdo(i,ki)
             if(zdo(i,ki+1).gt.0._r8)cdd(i,ki)=dd_massdetro(i,ki)/(dzo*zdo(i,ki+1))
          endif
          if(zdo(i,ki+1).gt.0._r8)mentrd_rate_2d(i,ki)=dd_massentro(i,ki)/(dzo*zdo(i,ki+1))
        enddo
        mentrd_rate_2d(i,1)=0._r8
        do ki=maxloc(zdo(i,:),1)-1,1,-1
          ...
          if(dd_massdetro(i,ki).lt.0._r8)then
            dd_massdetro(i,ki)=0._r8
        ...
        enddo
         ...
         cend=min(po_cup(i,ktop(i)),400._r8)
         cmid=.5_r8*(cbeg+cend) !600.
         ...
         do k=kbcon(i)+1,ktop(i)-1
           ...
           c1d(i,k)=max(0._r8,c1d(i,k))
           c1d(i,k)=c1
         enddo
         if(imid.eq.1)c1d(i,:)=0._r8  !WRF3.9
!        do k=1,jmin(i)
!         c1d(i,k)=0.
!        enddo
...
! downdraft moist static energy + moisture budget
            ...
            do k=2,jmin(i)+1 !WRF3.9
              dd_massentru(i,k-1)=dd_massentro(i,k-1)+lambau(i)*dd_massdetro(i,k-1) !WRF3.9
              dd_massdetru(i,k-1)=dd_massdetro(i,k-1)+lambau(i)*dd_massdetro(i,k-1) !WRF3.9
            enddo            !WRF3.9
            dbydo(i,jmin(i))=hcdo(i,jmin(i))-heso_cup(i,jmin(i))
            bud(i)=dbydo(i,jmin(i))*(zo_cup(i,jmin(i)+1)-zo_cup(i,jmin(i)))
            do ki=jmin(i)  ,1,-1
             dzo=zo_cup(i,ki+1)-zo_cup(i,ki)
             h_entr=.5_r8*(heo(i,ki)+.5_r8*(hco(i,ki)+hco(i,ki+1)))
             !WRF3.9:  detro->detru, entro->entru
             ucd(i,ki)=(ucd(i,ki+1)*zdo(i,ki+1)                       &
                         -.5_r8*dd_massdetru(i,ki)*ucd(i,ki+1)+ &
                        dd_massentru(i,ki)*us(i,ki)          &
                        -pgcon*zdo(i,ki+1)*(us(i,ki+1)-us(i,ki)))   /     &
                        (zdo(i,ki+1)-.5_r8*dd_massdetru(i,ki)+dd_massentru(i,ki))
             vcd(i,ki)=(vcd(i,ki+1)*zdo(i,ki+1)                       &
                         -.5_r8*dd_massdetru(i,ki)*vcd(i,ki+1)+ &
                        dd_massentru(i,ki)*vs(i,ki)            &
                        -pgcon*zdo(i,ki+1)*(vs(i,ki+1)-vs(i,ki)))   /  &
                        (zdo(i,ki+1)-.5_r8*dd_massdetru(i,ki)+dd_massentru(i,ki))
             !.. WRF3.9
             hcdo(i,ki)=(hcdo(i,ki+1)*zdo(i,ki+1)                       &
                         -.5_r8*dd_massdetro(i,ki)*hcdo(i,ki+1)+ &
                        dd_massentro(i,ki)*h_entr)   /            &
                        (zdo(i,ki+1)-.5_r8*dd_massdetro(i,ki)+dd_massentro(i,ki))
...
!
      DO i=its,itf
            if(ierr(i).eq.0)then
                ...
                wmean(i) = 7.0_r8 ! m/s ! in the future change for Wmean == integral( W dz) / cloud_depth
                if(imid.eq.1)wmean(i) = 3.0_r8
                !- time-scale cape removal from  Betchold et al. 2008
                ! org !!
                tau_ecmwf(i)=( zo_cup(i,ktopdby(i))- zo_cup(i,kbcon(i)) ) / wmean(i)
                tau_ecmwf(i)= tau_ecmwf(i) * (1.0061_r8 + 1.23E-2_r8 * (dx(i)/1000._r8))! dx(i) must be in meters
            endif
      enddo
      tau_bl(:itf)     = 0._r8
      !
      IF(dicycle == 1) then
        DO i=its,itf
            ...
            if(ierr(i).eq.0)then
                if(xland1(i) ==  0 ) then
                  !- over water
                  umean= 2.0_r8+sqrt(2.0_r8*(US(i,1)**2+VS(i,1)**2+US(i,kbcon(i))**2+VS(i,kbcon(i))**2))
        ...
        t_star=4.  !original =1
           ...
           !-- calculate pcape from BL forcing only
            ...
            if(ierr(i).eq.0)then
               ...
               !if(zo_cup(i,kbcon(i))-z1(i) > zo(i,kpbl(i)-1)) then  !WRF3.9 fr1
               !if(zo_cup(i,kbcon(i))-z1(i) > zo(i,kpbl(i)+1)) then   !WRF3.9
               if(zo_cup(i,kbcon(i)) > zo(i,kpbl(i)+1)) then   !JHJ Bug Fix
                  aa1_bl(i) = 0.0_r8
               else
               !- multiply aa1_bl the " time-scale" - tau_bl
                  aa1_bl(i) = max(0._r8,aa1_bl(i)/t_star* tau_bl(i))
               endif
            endif
            ENDDO
          ...
          DO i=its,itf
            if(ierr(i).eq.0)then
               do k=kbcon(i)+1,ktop(i)
                    hco_bl(i,k)=(hco_bl(i,k-1)*zuo(i,k-1)-.5_r8*up_massdetro(i,k-1)*hco_bl(i,k-1)+ &
...
!
      call cup_dd_edt(ierr,us,vs,zo,ktop,kbcon,edt,po,pwavo, &
        ...
        do k=kts,ktf
        do i=its,itf
           dellat_ens (i,k,1)=0._r8
           dellaq_ens (i,k,1)=0._r8
           dellaqc_ens(i,k,1)=0._r8
           pwo_ens    (i,k,1)=0._r8
        enddo
        enddo
!
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
         ...
         do k=kts+1,ktop(i)
            ...
            if(abs(totmas).gt.1.e-6_r8)then
               if ( masterproc ) then
               !write(0,123)'totmas=',k22(i),kbcon(i),k,entup,detup,zuo(i,k+1),zuo(i,k),detdo,entdo
               write(iulog,123)'totmas=',k22(i),kbcon(i),k,entup,detup,zuo(i,k+1),zuo(i,k),detdo,entdo
123     format(a7,1X,3i3,2E12.4,2(1x,f5.2),2e12.4)
               endif
            endif
            dp=100._r8*(po_cup(i,k)-po_cup(i,k+1))
             pgc=pgcon
            if(k.ge.ktop(i))pgc=0._r8
...
!WRF3.9 fr1           +(pgc*.5_r8*(zuo(i,k)+zuo(i,k+1))*(u_cup(i,k+1)-u_cup(i,k)) &
!WRF3.9 fr1           + pgcd*pgc*.5_r8*edto(i)*(zdo(i,k)+zdo(i,k+1))*(u_cup(i,k+1)-u_cup(i,k)))*g/dp
...
!WRF3.9 fr1           +(pgc*.5_r8*(zuo(i,k)+zuo(i,k+1))*(v_cup(i,k+1)-v_cup(i,k)) &
!WRF3.9 fr1           + pgcd*pgc*.5_r8*edto(i)*(zdo(i,k)+zdo(i,k+1))*(v_cup(i,k+1)-v_cup(i,k)))*g/dp
    ...
    do i=its,itf
        ...
        if(ierr(i).eq.0)then
         ...
         dp=100._r8*(po_cup(i,1)-po_cup(i,2))
         ...
         G_rain=  0.5_r8*(pwo (i,1)+pwo (i,2))*g/dp
         E_dn  = -0.5_r8*(pwdo(i,1)+pwdo(i,2))*g/dp*edto(i)  ! pwdo < 0 and E_dn must > 0
         ...
         do k=kts+1,ktop(i)
            ...
            detup=up_massdetro(i,k)
            !dz=zo_cup(i,k+1)-zo_cup(i,k) !WRF3.9 fr1
            dz=zo_cup(i,k)-zo_cup(i,k-1)  !WRF3.9
            if(k.lt.ktop(i)) dellaqc(i,k) = zuo(i,k)*c1d(i,k)*qrco(i,k)*dz/dp*g
!             dellaqc(i,k)=  detup*0.5*(qrco(i,k+1)+qrco(i,k)) *g/dp
            if(k.eq.ktop(i)) dellaqc(i,k) = detup*0.5_r8*(qrco(i,k+1)+qrco(i,k)) *g/dp
            !---

            G_rain=  0.5_r8*(pwo (i,k)+pwo (i,k+1))*g/dp
            E_dn  = -0.5_r8*(pwdo(i,k)+pwdo(i,k+1))*g/dp*edto(i) ! pwdo < 0 and E_dn must > 0
...
!            C_up = dellaqc(i,k)+ G_rain
            ...
            !               (Mu*qc+Md*qc)-C_up+E_dn +dellaqc +G_rain-E_dn
            !                             ----
            !                            -(dellaqc+Mu*qrc+G_rain)
            !               (Mu*qc+Md*qc)-Mu*qrc
...
!            XQ(I,K)=max(1.e-16,(dellaqc(i,k)+DELLAQ(I,K))*MBDT+QO(I,K))
            XQ(I,K)=max(1.e-16_r8,DELLAQ(I,K)*MBDT+QO(I,K))
            DELLAT(I,K)=(1._r8/cp)*(DELLAH(I,K)-xlv*DELLAQ(I,K))
!            XT(I,K)= (DELLAT(I,K)-xlv/cp*dellaqc(i,k))*MBDT+TN(I,K)
            XT(I,K)= DELLAT(I,K)*MBDT+TN(I,K)
            xt(i,k)=max(190._r8,xt(i,k))
           enddo
         ENDIF
      enddo
      do i=its,itf
      ...
      do i=its,itf
       if(ierr(i).eq.0)then
        do k=start_level(i)  +1,ktop(i)
         xhc(i,k)=(xhc(i,k-1)*xzu(i,k-1)-.5_r8*up_massdetro(i,k-1)*xhc(i,k-1)  + &
                                            up_massentro(i,k-1)*xhe(i,k-1)) / &
                             (xzu(i,k-1)-.5_r8*up_massdetro(i,k-1)+up_massentro(i,k-1))
...
!--- b=beta/2
                 ...
                 enddo
           enddo
         if(pr_ens(i,7).lt.1.e-6_r8)then
            ierr(i)=18
            ierrc(i)="total normalized condensate too small"
            do nens3=1,maxens3
               pr_ens(i,nens3)=0._r8
            enddo
         endif
         do nens3=1,maxens3
           if(pr_ens(i,nens3).lt.1.e-5_r8)then
            pr_ens(i,nens3)=0._r8
           endif
         enddo
         endif
...
!--JHJ
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
           ...
           pwo_ens    (i,k,1)=pwo(i,k) !+edto(i)*pwdo(i,k) !WRF3.9
           !pwo_ens    (i,k,1)=pwo(i,k)+edto(i)*pwdo(i,k)  !WRF3.9 fr1
        else
           dellat_ens (i,k,1)=0._r8
           dellaq_ens (i,k,1)=0._r8
           dellaqc_ens(i,k,1)=0._r8
...
!
         ...
         do i=its,itf
          ...
          if(ierr(i).eq.0)then
            ...
            if(k22(i).lt.kpbl(i)+1)then
               do k=1,kpbl(i)
                  blqe=blqe+100._r8*dhdt(i,k)*(po_cup(i,k)-po_cup(i,k+1))/g
               enddo
               trash=max((hco(i,kbcon(i))-heo_cup(i,kbcon(i))),1.e1_r8)
               xff_mid(i,1)=max(0._r8,blqe/trash)
               xff_mid(i,1)=min(0.1_r8,xff_mid(i,1))
             endif
             xff_mid(i,2)=min(0.1_r8,.03_r8*zws(i))
          endif
         enddo
       endif
       call cup_output_ens_3d(xff_mid,xf_ens,ierr,dellat_ens,dellaq_ens, &
            ...
            !'deep',ierr2,ierr3,                  & !WRF3.9 fr1
            !pr_ens,maxens3,                      & !WRF3.9 fr1
            edto,pwdo,'deep',ierr2,ierr3,         & !WRF3.9
            po_cup,pr_ens,maxens3,                & !WRF3.9
            sig,closure_n,xland1,xmbm_in,xmbs_in,   &
            ichoice,imid,ipr,itf,ktf,                        &
            its,ite, kts,kte, &
            dicycle,xf_dicycle,rprd )

!++JHJ
      do i=its,itf
         sig_out(i) = sig(i)
      do k=1,maxens3
         xf_ens2_out(i,k) = xf_ens(i,k)
      enddo
      enddo
!--JHJ
      k=1
      do i=its,itf
          if(ierr(i).eq.0 .and.pre(i).gt.0._r8) then
             PRE(I)=MAX(PRE(I),0._r8)
             xmb_out(i)=xmb(i)
             do k=kts,ktop(i)
               outu(i,k)=dellu(i,k)*xmb(i)
               outv(i,k)=dellv(i,k)*xmb(i)
             enddo
          elseif(ierr(i).ne.0 .or. pre(i).eq.0._r8)then
             ktop(i)=0
             do k=kts,kte
               outt(i,k)=0._r8
               outq(i,k)=0._r8
      ...
      do i = its,itf
       ...
       if(ierr(i).eq.0)then
         do k = ktop(i), 1, -1
              ...
              rntot(i) = rntot(i) + rain * xmb(i)* .001_r8 * dtime
         enddo
       endif
      enddo
      do i = its,itf
         ...
         if(ierr(i).eq.0)then
         evef = edt(i) * evfact
         if(xland(i).gt.0.5_r8 .and. xland(i).lt.1.5_r8) evef=edt(i) * evfactl
         do k = ktop(i), 1, -1
              rain =  pwo(i,k) + edto(i) * pwdo(i,k)
              rn(i) = rn(i) + rain * xmb(i) * .001_r8 * dtime
              if(flg(i))then
              q1=qo(i,k)+(outq(i,k))*dtime
              t1=tn(i,k)+(outt(i,k))*dtime
              qcond(i) = evef * (q1 - qeso(i,k))            &
     &                 / (1._r8 + el2orc * qeso(i,k) / t1**2)
              dp = -100._r8*(p_cup(i,k+1)-p_cup(i,k))
              if(rn(i).gt.0._r8 .and. qcond(i).lt.0._r8) then
                qevap(i) = -qcond(i) * (1._r8-exp(-.32_r8*sqrt(dtime*rn(i))))
                qevap(i) = min(qevap(i), rn(i)*1000._r8*g/dp)
                delq2(i) = delqev(i) + .001_r8 * qevap(i) * dp / g
              endif
              if(rn(i).gt.0._r8.and.qcond(i).lt.0._r8.and. &
     &           delq2(i).gt.rntot(i)) then
                qevap(i) = 1000._r8* g * (rntot(i) - delqev(i)) / dp
                flg(i) = .false.
              endif
              if(rn(i).gt.0._r8.and.qevap(i).gt.0._r8) then
                outq(i,k) = outq(i,k) + qevap(i)/dtime
                outt(i,k) = outt(i,k) - elocp * qevap(i)/dtime
                rn(i) = max(0._r8,rn(i) - .001_r8 * qevap(i) * dp / g)
                pre(i) = pre(i) - qevap(i) * dp /g/dtime
                PRE(I)=MAX(PRE(I),0._r8)
                delqev(i) = delqev(i) + .001_r8*dp*qevap(i)/g
!++JHJ
                evap_out(i,k) = qevap(i)/dtime
!--JHJ
              endif
              endif !flg
        enddo
...
!
      do i=its,itf
          if(ierr(i).eq.0) then
             dts=0._r8
             fpi=0._r8
             !do k=kts+1,ktop(i) !WRF3.9 fr1
                !dp=-.5_r8*(po(i,k+1)-po(i,k-1))*100._r8 !WRF3.9 fr1
             do k=kts,ktop(i)
                dp= (po_cup(i,k)-po_cup(i,k+1))*100._r8 !WRF3.9
!total KE dissiptaion estimate
                !dts= dts +(outu(i,k)*us(i,k)+outv(i,k)*vs(i,k))*dp/g !WRF3.9 fr1
                dts= dts -(outu(i,k)*us(i,k)+outv(i,k)*vs(i,k))*dp/g  !WRF3.9
! fpi needed for calcualtion of conversion to pot. energyintegrated
                fpi = fpi  +sqrt(outu(i,k)*outu(i,k) + outv(i,k)*outv(i,k))*dp
             enddo
             if(fpi.gt.0._r8)then
                !do k=kts+1,ktop(i) !WRF3.9 fr1
                do k=kts,ktop(i)    !WRF3.9
...
!++JHJ
      ...
      !   !do k=kts,ktf
      !   do k=kts,ktop(i)
      !      dp = (po_cup(i,k)-po_cup(i,k+1))*100._r8
      !      rliq(i) = rliq(i) + dlf(i,k)*dp/g
      !   enddo
      !enddo
      !rliq(:itf) = rliq(:itf) /1000._r8
...
!
     ...
     integer i,k,kk
     real(r8)    einc,pef,pefb,prezk,zkbc
     real(r8),    dimension (pcols)         ::                           &
      vshear,sdp,vws
     real(r8) :: prop_c,pefc,aeroadd,alpha3,beta3
     prop_c = 8._r8 !10.386
     alpha3 = 1.9_r8
     beta3  = -1.13_r8
     pefc=0._r8
...
!
       ...
       do kk = kts,ktf-1
         do 62 i=its,itf
          ...
          if (kk .eq. ktf-1)vshear(i) = 1.e3_r8 * vws(i) / sdp(i)
   62   continue
       end do
      do i=its,itf
         IF(ierr(i).eq.0)then
            pef=(1.591_r8-.639_r8*VSHEAR(I)+.0953_r8*(VSHEAR(I)**2) &
               -.00496_r8*(VSHEAR(I)**3))
            if(pef.gt.0.9_r8)pef=0.9_r8
            if(pef.lt.0.1_r8)pef=0.1_r8
!
!--- cloud base precip efficiency
!
            zkbc=z(i,kbcon(i))*3.281e-3_r8
            prezk=.02_r8
            if(zkbc.gt.3._r8)then
               prezk=.96729352_r8+zkbc*(-.70034167_r8+zkbc*(.162179896_r8+zkbc &
               *(- 1.2569798E-2_r8+zkbc*(4.2772E-4_r8-zkbc*5.44E-6_r8))))
            endif
            if(zkbc.gt.25)then
               prezk=2.4_r8
            endif
            pefb=1._r8/(1._r8+prezk)
            if(pefb.gt.0.9_r8)pefb=0.9_r8
            if(pefb.lt.0.1_r8)pefb=0.1_r8
            EDT(I)=1._r8-.5_r8*(pefb+pef)
            if(aeroevap.gt.1)then
               aeroadd=(ccnclean**beta3)*((psumh(i))**(alpha3-1)) !*1.e6
!              prop_c=.9_r8/aeroadd
               prop_c=.5_r8*(pefb+pef)/aeroadd
               aeroadd=(ccn(i)**beta3)*((psum2(i))**(alpha3-1)) !*1.e6
               aeroadd=prop_c*aeroadd
               pefc=aeroadd
               if(pefc.gt.0.9_r8)pefc=0.9_r8
               if(pefc.lt.0.1_r8)pefc=0.1_r8
               EDT(I)=1._r8-pefc
               if(aeroevap.eq.2)EDT(I)=1._r8-.25_r8*(pefb+pef+2._r8*pefc)
            endif
...
!
      ...
      if(dh.lt.0)then
        QRCD(I,K)=(qes_cup(i,k)+(1./XLV)*(GAMMA_cup(i,k) &
                  /(1._r8+GAMMA_cup(i,k)))*DH)
        else
          qrcd(i,k)=qes_cup(i,k)
        endif
      pwd(i,jmin(i))=zd(i,jmin(i))*min(0._r8,qcd(i,k)-qrcd(i,k))
      qcd(i,k)=qrcd(i,k)
...
!JOE-added check for non-zero denominator:
         denom=zd(i,ki+1)-.5_r8*dd_massdetr(i,ki)+dd_massentr(i,ki)
         if(denom.lt.1.e-8_r8)then
            ierr(i)=51
            exit
         endif
         qcd(i,ki)=(qcd(i,ki+1)*zd(i,ki+1)                          &
                  -.5_r8*dd_massdetr(i,ki)*qcd(i,ki+1)+ &
                  dd_massentr(i,ki)*q(i,ki))   /            &
                  (zd(i,ki+1)-.5_r8*dd_massdetr(i,ki)+dd_massentr(i,ki))
!
...
!
         ...
         bu(i)=bu(i)+dz*dh
         QRCD(I,Ki)=qes_cup(i,ki)+(1._r8/XLV)*(GAMMA_cup(i,ki) &
                  /(1._r8+GAMMA_cup(i,ki)))*DH
         dqeva=qcd(i,ki)-qrcd(i,ki)
         if(dqeva.gt.0._r8)then
          dqeva=0._r8
          qrcd(i,ki)=qcd(i,ki)
         endif
...
!
!--- end loop over i
       if( (pwev(i).eq.0._r8) .and. (iloop.eq.1))then
...
!
     ...
     integer                              ::                           &
       i,k
!     real(r8), dimension (1:2) :: AE,BE,HT
      real(r8), dimension (pcols,kts:kte) :: tv
      real(r8) :: tcrit,e,tvbar
!     real(r8), external :: satvap
!     real(r8) :: satvap
...
!       QES(I,K)=.622*E/(100.*P(I,K)-E)
        e=satvap(t(i,k))
        qes(i,k)=0.622_r8*e/max(1.e-8_r8,(p(i,k)-e))
        IF(QES(I,K).LE.1.E-16_r8)QES(I,K)=1.E-16_r8
        IF(QES(I,K).LT.Q(I,K))QES(I,K)=Q(I,K)
!       IF(Q(I,K).GT.QES(I,K))Q(I,K)=QES(I,K)
        TV(I,K)=T(I,K)+.608_r8*Q(I,K)*T(I,K)
        endif
      enddo
      enddo
!
...
!
      if(itest.eq.1 .or. itest.eq.0)then
         do i=its,itf
           if(ierr(i).eq.0)then
!++JHJ
             !Z(I,1)=max(0.,Z1(I))-(ALOG(P(I,1))- & !org
             !    ALOG(PSUR(I)))*287.*TV(I,1)/9.81  !org
             Z(I,1)=max(0._r8,Z1(I))-(LOG(P(I,1))- &
                 LOG(PSUR(I)))*287._r8*TV(I,1)/9.81_r8
!--JHJ
           endif
         enddo
...
! --- calculate heights
         ...
         do i=its,itf
           if(ierr(i).eq.0)then
              TVBAR=.5_r8*TV(I,K)+.5_r8*TV(I,K-1)
!++JHJ
              !Z(I,K)=Z(I,K-1)-(ALOG(P(I,K))- &   !org
              ! ALOG(P(I,K-1)))*287.*TVBAR/9.81   !org
              Z(I,K)=Z(I,K-1)-(LOG(P(I,K))- &
               LOG(P(I,K-1)))*287._r8*TVBAR/9.81_r8
!--JHJ
           endif
         enddo
         enddo
         ...
         do i=its,itf
           if(ierr(i).eq.0)then
             z(i,k)=(he(i,k)-1004._r8*t(i,k)-2.5e6_r8*q(i,k))/9.81_r8
             z(i,k)=max(1.e-3_r8,z(i,k))
           endif
         enddo
         enddo
...
!
       ...
       do i=its,itf
         if(ierr(i).eq.0)then
         if(itest.le.0)HE(I,K)=9.81_r8*Z(I,K)+1004._r8*T(I,K)+2.5E06_r8*Q(I,K)
         HES(I,K)=9.81_r8*Z(I,K)+1004._r8*T(I,K)+2.5E06_r8*QES(I,K)
...
!
     ...
     integer                              ::                           &
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
        qes_cup(i,k)=.5_r8*(qes(i,k-1)+qes(i,k))
        q_cup(i,k)=.5_r8*(q(i,k-1)+q(i,k))
        hes_cup(i,k)=.5_r8*(hes(i,k-1)+hes(i,k))
        he_cup(i,k)=.5_r8*(he(i,k-1)+he(i,k))
        if(he_cup(i,k).gt.hes_cup(i,k))he_cup(i,k)=hes_cup(i,k)
        z_cup(i,k)=.5_r8*(z(i,k-1)+z(i,k))
        p_cup(i,k)=.5_r8*(p(i,k-1)+p(i,k))
        t_cup(i,k)=.5_r8*(t(i,k-1)+t(i,k))
...
!       hes_cup(i,1)=hes(i,1)
!       he_cup(i,1)=he(i,1)
        hes_cup(i,1)=9.81_r8*z1(i)+1004._r8*t(i,1)+2.5e6_r8*qes(i,1)
        he_cup(i,1)=9.81_r8*z1(i)+1004._r8*t(i,1)+2.5e6_r8*q(i,1)
        z_cup(i,1)=.5_r8*(z(i,1)+z1(i))
        p_cup(i,1)=.5_r8*(p(i,1)+psur(i))
        z_cup(i,1)=z1(i)
        p_cup(i,1)=psur(i)
        t_cup(i,1)=t(i,1)
  ...
  !
     ...
     integer                                                           &
        ...
        ichoice
      integer, intent(IN) :: DICYCLE
      real(r8),    intent(IN)   , dimension (pcols) :: aa1_bl,tau_ecmwf
      real(r8),    intent(INOUT), dimension (pcols) :: xf_dicycle
      real(r8),    intent(INOUT), dimension (pcols,10) :: forcing
      !- local var
      real(r8)  :: xff_dicycle
!
...
!     real(r8),    dimension(1:mkxcrt)         ::                           &
!       pcrit,acrit,acritt
     integer, dimension (pcols)         :: kloc
     real(r8)                                 ::                           &
       a1,a_ave,xff0,xomg!,aclim1,aclim2,aclim3,aclim4

     real(r8), dimension (pcols) :: ens_adj
...
!ss --- comment out adjustment over ocean
           !if(ierr2(i).gt.0.and.ierr3(i).eq.0)ens_adj(i)=0._r8 ! WRF3.9 fr1
           !if(ierr2(i).gt.0.and.ierr3(i).gt.0)ens_adj(i)=0._r8 ! WRF3.9 fr1
           !if(ierr2(i).gt.0.and.ierr3(i).eq.0)ens_adj(i)=0.666_r8 !WRF3.9 but not used
           !if(ierr2(i).gt.0.and.ierr3(i).gt.0)ens_adj(i)=0.333_r8 !WRF3.9 but not used
!
             a_ave=0._r8
             a_ave=axx(i)
             a_ave=max(0._r8,a_ave)
             a_ave=min(a_ave,aa1(i))
             a_ave=max(0._r8,a_ave)
             xff_ens3(1:maxens3)=0._r8
             xff0= (AA1(I)-AA0(I))/DTIME
             xff_ens3(1)=max(0._r8,(AA1(I)-AA0(I))/dtime)
             xff_ens3(2)=max(0._r8,(AA1(I)-AA0(I))/dtime)
             xff_ens3(3)=max(0._r8,(AA1(I)-AA0(I))/dtime)
             xff_ens3(16)=max(0._r8,(AA1(I)-AA0(I))/dtime)
...
!
             ...
             do k=kbcon(i)-1,kbcon(i)+1
                     if(zu(i,k).gt.0._r8)then
                       xomg=xomg-omeg(i,k)/9.81_r8/max(0.5_r8,(1._r8-edt(i)*zd(i,k)/zu(i,k)))
                       kk=kk+1
                     endif
             enddo
...
!             if(zu(i,kbcon(i)) > 0)xff_ens3(6)=betajb*xff_ens3(6)/zu(i,kbcon(i))
             ...
             if(xff_ens3(4).lt.0._r8)xff_ens3(4)=0._r8
             if(xff_ens3(5).lt.0._r8)xff_ens3(5)=0._r8
             if(xff_ens3(6).lt.0._r8)xff_ens3(6)=0._r8
...
!
              xff_ens3(7)= mconv(i)/max(0.5_r8,(1._r8-edt(i)*zd(i,kbcon(i))/zu(i,kloc(i))))
              xff_ens3(8)= mconv(i)/max(0.5_r8,(1._r8-edt(i)*zd(i,kbcon(i))/zu(i,kloc(i))))
              xff_ens3(9)= mconv(i)/max(0.5_r8,(1._r8-edt(i)*zd(i,kbcon(i))/zu(i,kloc(i))))
              xff_ens3(15)=mconv(i)/max(0.5_r8,(1._r8-edt(i)*zd(i,kbcon(i))/zu(i,kloc(i))))
...
!- more like Bechtold et al. (JAS 2014)
             if(dicycle == 1) xff_dicycle = max(0._r8,AA1_BL(i)/tau_ecmwf(i)) !(60.*30.) !tau_ecmwf(i)
!gtest
             if(ichoice.eq.0)then
                if(xff0.lt.0._r8)then
                     xff_ens3(1)=0._r8
                     xff_ens3(2)=0._r8
                     xff_ens3(3)=0._r8
                     xff_ens3(10)=0._r8
                     ...
                     xff_ens3(16)= 0._r8
                     closure_n(i) = 12._r8
!WRF3.9 fr1          xff_dicycle = 0._r8
                endif  !xff0
                  !if(xff0.lt.0 .and. xland(i).lt.0.1_r8)then !WRF3.9 fr1
                  !   xff_ens3(1:maxens3)=0._r8               !WRF3.9 fr1
                  !endif                                      !WRF3.9 fr1
             endif ! ichoice
             ...
             forcing(i,7)=xk(1)
             if(xk(1).le.0._r8 .and. xk(1).gt.-.01_r8*mbdt) &
                           xk(1)=-.01_r8*mbdt
             if(xk(1).gt.0._r8 .and. xk(1).lt.1.e-2_r8) &
                           xk(1)=1.e-2_r8
             !   enddo
!
...
!
              if(xland(i).lt.0.1_r8)then
                 if(ierr2(i).gt.0.or.ierr3(i).gt.0)then
                      ...
                      xff_ens3(4) =ens_adj(i)*xff_ens3(4) !WRF3.9 used
                      xff_ens3(5) =ens_adj(i)*xff_ens3(5) !WRF3.9 used
                      xff_ens3(6) =ens_adj(i)*xff_ens3(6) !WRF3.9 used
                      ...
                      xff_ens3(14) =ens_adj(i)*xff_ens3(14) !WRF3.9 used
...
!
              if(XK(1).lt.0._r8)then
                 if(xff_ens3(1).gt.0._r8) xf_ens(i,1)=max(0._r8,-xff_ens3(1)/xk(1))
                 if(xff_ens3(2).gt.0._r8) xf_ens(i,2)=max(0._r8,-xff_ens3(2)/xk(1))
                 if(xff_ens3(3).gt.0._r8) xf_ens(i,3)=max(0._r8,-xff_ens3(3)/xk(1))
                 if(xff_ens3(16).gt.0._r8)xf_ens(i,16)=max(0._r8,-xff_ens3(16)/xk(1))
...
!
              xf_ens(i,4)=max(0._r8,xff_ens3(4))
              xf_ens(i,5)=max(0._r8,xff_ens3(5))
              xf_ens(i,6)=max(0._r8,xff_ens3(6))
              xf_ens(i,14)=max(0._r8,xff_ens3(14))
              a1=max(1.e-5_r8,pr_ens(i,7))
              xf_ens(i,7)=max(0._r8,xff_ens3(7)/a1)
              a1=max(1.e-5_r8,pr_ens(i,8))
              xf_ens(i,8)=max(0._r8,xff_ens3(8)/a1)
!              forcing(i,7)=xf_ens(i,8)
              a1=max(1.e-5_r8,pr_ens(i,9))
              xf_ens(i,9)=max(0._r8,xff_ens3(9)/a1)
              a1=max(1.e-3_r8,pr_ens(i,15))
              xf_ens(i,15)=max(0._r8,xff_ens3(15)/a1)
              ...
              if(XK(1).lt.0._r8)then
                 xf_ens(i,10)=max(0._r8,-xff_ens3(10)/xk(1))
                 xf_ens(i,11)=max(0._r8,-xff_ens3(11)/xk(1))
                 xf_ens(i,12)=max(0._r8,-xff_ens3(12)/xk(1))
                 xf_ens(i,13)=max(0._r8,-xff_ens3(13)/xk(1))
...
!srf-begin
              if(XK(1).lt.0._r8)then
                 xf_dicycle(i)      =  max(0._r8,-xff_dicycle /xk(1))
...
!                 closure_n(i)=0._r8 !WRF3.9 not used
...
!
  ...
  !
     ...
     integer                                                           &
        ...
        iloop_in
     character*50 :: ierrc(pcols)
     real(r8), dimension (pcols,kts:kte),intent (in) :: z_cup,heo
     integer, dimension (pcols)      ::     iloop,start_level
!
!  local variables in this routine
!
     ...
     real(r8), dimension (pcols,kts:kte) ::hcot
!
!--- DETERMINE THE LEVEL OF CONVECTIVE CLOUD BASE  - KBCON
!
      iloop(:itf)=iloop_in
...
! reset iloop for mid level convection
      if(cap_max(i).gt.200._r8 .and. imid.eq.1)iloop(i)=5
!
...
!      if(iloop_in.eq.5)start_level(i)=kbcon(i)
       !== including entrainment for hetest
        ...
        do k=start_level(i)+1,KBMAX(i)+3
           ...
           hcot(i,k)= ( (1._r8-0.5_r8*entr_rate(i)*dz)*hcot(i,k-1)   &
                         + entr_rate(i)*dz*heo(i,k-1)       )/ &
                      (1._r8+0.5_r8*entr_rate(i)*dz)
        enddo
       !==
...
!
        ...
        DO 100 i=its,itf
         ...
         DZ=Z(I,K)-Z(I,K-1)
         da=zu(i,k)*DZ*(9.81_r8/(1004._r8*( &
                (T_cup(I,K)))))*DBY(I,K-1)/ &
             (1._r8+GAMMA_CUP(I,K))
!         IF(K.eq.KTOP(I).and.da.le.0.)go to 100
         AA0(I)=AA0(I)+max(0._r8,da)
         if(aa0(i).lt.0._r8)aa0(i)=0._r8
100     continue
...
!====================================================================
   ...
   INTEGER,      INTENT(IN   ) ::            j,its,ite,kts,kte,itf,ktf
     ...
     real(r8)                                                                     &
        ,intent (in  )                   ::                                   &
        dt
     real(r8) :: names,scalef,thresh,qmem,qmemf,qmem2,qtest,qmem1
     integer :: icheck
!
...
!          print *,'1',' adjusted massflux by factor ',i,j,k,qmem,qmem2,qmemf,dt
           write(iulog,*) 'neg t1 adjusted massflux by factor ',i,j,k,qmem,qmem2,qmemf,dt
         endif
         if(qmem.lt.-.5_r8*thresh*names)then
           qmem2=-.5_r8*names*thresh/qmem
           qmemf=min(qmemf,qmem2)
           icheck=2
!
!
           write(iulog,*) 'neg t2 adjusted massflux by factor ',i,j,k,qmem,qmem2,qmemf,dt
         endif
      enddo
      do k=kts,ktf
...
!      write(14,*)'return'
      ...
      do k=kts,ktf-1
         qmem=outq(i,k)
         if(abs(qmem).gt.0._r8 .and. q(i,k).gt.1.e-6_r8)then
...
!
           ...
           qmemf=max(0._r8,qmemf)
           write(iulog,*) 'neg q1 adjusted massflux by factor ',i,j,k,qmem,qmem2,qmemf,dt
         endif
         endif
      enddo
      do k=kts,ktf
   ...
   SUBROUTINE cup_output_ens_3d(xff_mid,xf_ens,ierr,dellat,dellaq,dellaqc,  &
              outtem,outq,outqc,     &
              zu,pre,pw,xmb,ktop,                 &
              !name,ierr2,ierr3,pr_ens,             & !WRF3.9 fr1
              edt,pwd,name,ierr2,ierr3,p_cup,pr_ens,   & !WRF3.9
              maxens3,                            &
              sig,closure_n,xland1,xmbm_in,xmbs_in,    &
              ichoice,imid,ipr,itf,ktf, &
              its,ite, kts,kte, &
!++JHJ
              dicycle,xf_dicycle, & !org
              rprd)
!--JHJ
...
!
  ...
  !
     ...
     real(r8),   dimension (pcols)                                      &
         ,intent (in  )                   ::                           &
        sig,xmbm_in,xmbs_in,edt !WRf3.9
        !sig,xmbm_in,xmbs_in !WRF3.9 fr1
     real(r8),   dimension (pcols,2)                                      &
         ,intent (in  )                   ::                           &
        xff_mid
     real(r8),    dimension (pcols)                                      &
        ,intent (inout  )                   ::                           &
     ...
     integer, dimension (pcols)                                      &
        ...
        ierr,ierr2,ierr3
     integer, intent(IN) :: DICYCLE
     real(r8),    intent(IN), dimension (pcols) :: xf_dicycle
!++JHJ
     real(r8),    dimension (pcols,kts:kte)                              &
        ,intent (inout)                   ::                           &
        rprd

     integer :: nn,ichoice_mid1
!--JHJ
!
!  local variables in this routine
!
     ...
     integer                              ::                           &
        i,k,n
     real(r8)                                 ::                           &
        clos_wei,dp,dtt,dtq,dtqc,dtpw,dtpwd !WRF3.9
        !dtt,dtq,dtqc,dtpw !WRF3.9 fr1
     real(r8),    dimension (pcols)::                           &
       xmb_ave,pwtot  !WRF3.9
       !xmb_ave !WRF3.9 fr1
!
      character *(*), intent (in)        ::                           &
       name
...
!!!!! DEEP Convection !!!!!!!!!!
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
         ...
         !srf begin
         if(dicycle == 2 )then
            xmb_ave(i)=xmb_ave(i)-max(0._r8,xmbm_in(i),xmbs_in(i))
            xmb_ave(i)=max(0._r8,xmb_ave(i))
         else if (dicycle == 1) then
            xmb_ave(i)=min(xmb_ave(i),xmb_ave(i) - xf_dicycle(i))
            xmb_ave(i)=max(0._r8,xmb_ave(i))
         endif
         !xmb_ave(i)=min(xmb_ave(i),100._r8) !WRF3.9 fr1
         !xmb(i)=sig(i)*xmb_ave(i)           !WRF3.9 fr1
...
!       closures over water) to properly normalize xmb
           clos_wei=16./max(1.,closure_n(i))  !WRF3.9 used
           xmb_ave(i)=min(xmb_ave(i),100._r8) !WRF3.9
           xmb(i)=clos_wei*sig(i)*xmb_ave(i)  !WRF3.9


           if(xmb(i) < 1.e-16_r8)then
              ierr(i)=19
           endif
!           xfac1(i)=xmb(i)
!           xfac2(i)=xmb(i)
...
!++JHJ
              if (ichoice_mid1 .eq. 2) then
              xmb_ave(i)=sig(i)*xff_mid(i,2)
              else
!--JHJ
              xmb_ave(i)=.5_r8*sig(i)*(xff_mid(i,1)+xff_mid(i,2))
              endif !JHJ
           endif   ! ichoice gt.2
! which dicycle method
           if(dicycle == 2 )then
              xmb(i)=max(0._r8,xmb_ave(i)-xmbs_in(i))
           else if (dicycle == 1) then
              xmb(i)=min(xmb_ave(i),xmb_ave(i) - xf_dicycle(i))
              xmb(i)=max(0._r8,xmb_ave(i))
      ...
      do i=its,itf
        ...
        IF(ierr(i).eq.0)then
            DO k=kts,ktop(i)                !WRF3.9
               pwtot(i)=pwtot(i)+pw(i,k,1)  !WRF3.9
            enddo                           !WRF3.9
            DO k=kts,ktop(i)
! WRF3.9 fr
!            dtt =dellat  (i,k,1)
...
! WRF3.9
             dp=100._r8*(p_cup(i,k)-p_cup(i,k+1))/g
             dtt =dellat  (i,k,1)
             dtq =dellaq  (i,k,1)
...
!           PRE(I)=PRE(I)+XMB(I)*dtpw !WRF3.9 fr1
           xf_ens(i,1:maxens3)=sig(i)*xf_ens(i,1:maxens3)

!what is evaporated
           PRE(I)=PRE(I)+XMB(I)*dtpwd !WRF3.9

           rprd(i,k) = ( XMB(I)*pw(i,k,1) - XMB(I)*dtpwd )/dp ![kg/kg/s]
           rprd(i,k)=MAX(rprd(i,k),0._r8)

          enddo
          PRE(I)=-PRE(I)+XMB(I)*pwtot(i) !WRF3.9
        endif
      enddo
...
!-------------------------------------------------------
   ...
   IMPLICIT NONE
  real(r8), parameter :: BDISPM = 0.366_r8       !Berry--size dispersion (martime)
  real(r8), PARAMETER :: BDISPC = 0.146_r8       !Berry--size dispersion (continental)
!
!  on input
!
...
!
     ...
     integer                              ::                           &
        iprop,iall,i,k
     integer :: start_level(pcols)
     real(r8)                                 ::                           &
        prop_ave,qrcb_h,bdsp,dp,rhoc,qrch,qaver, &
        c0,dz,berryc0,q1,berryc
     real(r8)                                 ::                           &
        denom
     real(r8),    dimension (kts:kte)         ::                           &
        prop_b
!
        prop_b(kts:kte)=0._r8
        iall=0
        !c0=.004_r8 !WRF3.9 fr1
        c0=.002_r8 !WRF3.9
        bdsp=BDISPM
!
...
!            if(name == "deep" )then
            DO k=k22(i)+1,kbcon(i)
              qc(i,k)=   (qc(i,k-1)*zu(i,k-1)-.5_r8*up_massdetr(i,k-1)* qc(i,k-1)+ &
                         up_massentr(i,k-1)*q(i,k-1))   /            &
                         (zu(i,k-1)-.5_r8*up_massdetr(i,k-1)+up_massentr(i,k-1))
!              QRCH=QES_cup(I,K)
               QRCH=QES_cup(I,K)+(1._r8/XLV)*(GAMMA_cup(i,k) &
                 /(1._r8+GAMMA_cup(i,k)))*DBY(I,K)
              if(k.lt.kbcon(i))qrch=qc(i,k)
              if(qc(i,k).gt.qrch)then
                DZ=Z_cup(i,K)-Z_cup(i,K-1)
                QRC(I,K)=(QC(I,K)-QRCH)/(1._r8+c0*DZ)
...
!
            DO k=kbcon(i)+1,ktop(i)
               c0=.004_r8
               if(t(i,k).lt.270._r8)c0=.002_r8
               denom=zu(i,k-1)-.5_r8*up_massdetr(i,k-1)+up_massentr(i,k-1)
               if(denom.lt.1.e-8_r8)then
                     ierr(i)=51
                exit
               endif
...
!
               qc(i,k)=   (qc(i,k-1)*zu(i,k-1)-.5_r8*up_massdetr(i,k-1)* qc(i,k-1)+ &
                         up_massentr(i,k-1)*q(i,k-1))   /            &
                         (zu(i,k-1)-.5_r8*up_massdetr(i,k-1)+up_massentr(i,k-1))
               qch(i,k)= (qch(i,k-1)*zu(i,k-1)-.5_r8*up_massdetr(i,k-1)*qch(i,k-1)+ &
                         up_massentr(i,k-1)*q(i,k-1))   /            &
                         (zu(i,k-1)-.5_r8*up_massdetr(i,k-1)+up_massentr(i,k-1))
...
!
               clw_all(i,k)=max(0._r8,QC(I,K)-QRCH)
               QRC(I,K)=max(0._r8,(QC(I,K)-QRCH)) ! /(1.+C0*DZ*zu(i,k))
               clw_allh(i,k)=max(0._r8,QCH(I,K)-QRCH)
               QRCB(I,K)=max(0._r8,(QCH(I,K)-QRCH)) ! /(1.+C0*DZ*zu(i,k))
...
!
                 q1=1.e3_r8*rhoc*qrcb(i,k)  ! g/m^3 ! g[h2o]/cm^3
                 berryc0=q1*q1/(60.0_r8*(5.0_r8 + 0.0366_r8*CCNclean/ &
                    ( q1 * BDSP)  ) ) !/(
                 qrcb_h=((QCH(I,K)-QRCH)*zu(i,k)-qrcb(i,k-1)*(.5_r8*up_massdetr(i,k-1)))/ &
                   (zu(i,k)+.5_r8*up_massdetr(i,k-1)+c0*dz*zu(i,k))
                 prop_b(k)=c0*qrcb_h*zu(i,k)/(1.e-3_r8*berryc0)
                 pwh(i,k)=zu(i,k)*1.e-3_r8*berryc0*dz*prop_b(k) ! 2.
                 berryc=qrcb(i,k)
                 qrcb(i,k)=((QCh(I,K)-QRCH)*zu(i,k)-pwh(i,k)-qrcb(i,k-1)*(.5_r8*up_massdetr(i,k-1)))/ &
                       (zu(i,k)+.5_r8*up_massdetr(i,k-1))
                 if(qrcb(i,k).lt.0._r8)then
                   berryc0=(qrcb(i,k-1)*(.5_r8*up_massdetr(i,k-1))-(QCh(I,K)-QRCH)*zu(i,k))/zu(i,k)*1.e-3_r8*dz*prop_b(k)
                   pwh(i,k)=zu(i,k)*1.e-3_r8*berryc0*dz*prop_b(k)
                   qrcb(i,k)=0._r8
                 endif
...
! then the real berry
!
                 q1=1.e3_r8*rhoc*qrc(i,k)  ! g/m^3 ! g[h2o]/cm^3
                 berryc0=q1*q1/(60.0_r8*(5.0_r8 + 0.0366_r8*CCN(i)/ &
                    ( q1 * BDSP)  ) ) !/(
                 berryc0=1.e-3_r8*berryc0*dz*prop_b(k) ! 2.
                 berryc=qrc(i,k)
                 qrc(i,k)=((QC(I,K)-QRCH)*zu(i,k)-zu(i,k)*berryc0-qrc(i,k-1)*(.5_r8*up_massdetr(i,k-1)))/ &
                       (zu(i,k)+.5_r8*up_massdetr(i,k-1))
                 if(qrc(i,k).lt.0._r8)then
                    berryc0=((QC(I,K)-QRCH)*zu(i,k)-qrc(i,k-1)*(.5_r8*up_massdetr(i,k-1)))/zu(i,k)
                    qrc(i,k)=0._r8
                 endif
...
!
               ELSE       !c0=.002
                 if(iall.eq.1)then
                   ...
                   if(pw(i,k).lt.0._r8)pw(i,k)=0._r8
                 else
                   QRC(I,K)=(QC(I,K)-QRCH)/(1._r8+(c1d(i,k)+C0)*DZ)
                   PW(i,k)=c0*dz*QRC(I,K)*zu(i,k)
...
!
 100     CONTINUE
       ...
       do k=kts,kte
        ...
        if(prop_b(k).gt.0._r8)iprop=iprop+1
       enddo
       iprop=max(iprop,1)
...
!--------------------------------------------------------------------
 ...
 REAL(r8) FUNCTION satvap(temp2)
      implicit none
      real(r8) :: temp2, temp, toot, toto, eilog, tsot,  &
     &        ewlog, ewlog2, ewlog3, ewlog4
      temp = temp2-273.155_r8
      if (temp.lt.-20._r8) then   !!!! ice saturation
        toot = 273.16_r8 / temp2
        toto = 1._r8 / toot
        eilog = -9.09718_r8 * (toot - 1._r8) - 3.56654_r8 * (log(toot) / &
     &    log(10._r8)) + .876793_r8 * (1._r8 - toto) + (log(6.1071_r8) / log(10._r8))
        satvap = 10._r8 ** eilog
      else
        tsot = 373.16_r8 / temp2
        ewlog = -7.90298_r8 * (tsot - 1._r8) + 5.02808_r8 * &
     &             (log(tsot) / log(10._r8))
        ewlog2 = ewlog - 1.3816e-07_r8 * &
     &             (10._r8 ** (11.344_r8 * (1._r8 - (1._r8 / tsot))) - 1._r8)
        ewlog3 = ewlog2 + .0081328 * &
     &             (10._r8 ** (-3.49149_r8 * (tsot - 1._r8)) - 1._r8)
        ewlog4 = ewlog3 + (log(1013.246_r8) / log(10._r8))
        satvap = 10._r8 ** ewlog4
      end if
 END FUNCTION
!--------------------------------------------------------------------
 SUBROUTINE get_cloud_bc(mzp,array,x_aver,k22,add)
    ...
    real(r8)   , intent(in)     :: array(mzp)
    real(r8)   , optional , intent(in)  :: add
    real(r8)   , intent(out)    :: x_aver
 ...
 SUBROUTINE rates_up_pdf(rand_vmas,ipr,name,ktop,ierr,p_cup,entr_rate_2d,hkbo,heo,heso_cup,z_cup, &
     ...
     real(r8), dimension (pcols,kts:kte),intent (inout) :: entr_rate_2d,zuo
     real(r8), dimension (pcols,kts:kte),intent (in) ::p_cup, heo,heso_cup,z_cup
     real(r8), dimension (pcols),intent (in) :: hkbo,rand_vmas
     integer, dimension (pcols),intent (in) :: kstabi,k22,kpbl,csum,xland,pmin_lev
     integer, dimension (pcols),intent (inout) :: kbcon,ierr,ktop,ktopdby
     !-local vars
     real(r8), dimension (pcols,kts:kte) :: hcot
     real(r8) :: beta_u,dz,dbythresh,dzh2,zustart,zubeg,massent,massdetr
     !real(r8) :: dby(kts:kte),zux(kts:kte) !WRF3.9 fr1
     real(r8) :: dby(kts:kte),dbm(kts:kte),zux(kts:kte)  !WRF3.9
     real(r8) zuh2(40),zh2(40)
     integer :: kklev,i,kk,kbegin,k,kfinalzu
     integer, dimension (pcols) :: start_level
     !
     zustart=.1_r8
     dbythresh= 1._r8 !.0.95 ! 0.85, 0.6
     if(name == 'shallow' .or. name == 'mid') dbythresh=1._r8
     dby(:)=0._r8
     dbm(:)=0._r8 !WRF3.9

     DO i=its,itf
      zux(:)=0._r8
      beta_u=max(.1_r8,.2_r8-float(csum(i))*.01_r8)
      zuo(i,:)=0._r8
      dby(:)=0._r8
      ...
      if(ierr(i).eq.0)then
       ...
       zuo(i,start_level(i))=zustart
        ...
        do k=start_level(i)+1,kbcon(i)
          ...
          massdetr=dz*1.e-9_r8*zuo(i,k-1)
       ...
       if(name .eq. 'deep')then
        ...
        do k=start_level(i)+1,ktf-2
           ...
           hcot(i,k)=( (1._r8-0.5_r8*entr_rate_2d(i,k-1)*dz)*hcot(i,k-1) &
                      + entr_rate_2d(i,k-1)*dz*heo(i,k-1))/ &
                      (1._r8+0.5_r8*entr_rate_2d(i,k-1)*dz)
           if(k >= kbcon(i)) dby(k)=dby(k-1)+(hcot(i,k)-heso_cup(i,k))*dz
           if(k >= kbcon(i)) dbm(k)=hcot(i,k)-heso_cup(i,k) !WRF3.9
        enddo
        ktopdby(i)=maxloc(dby(:),1)
        kklev=maxloc(dbm(:),1) !WRF3.9
...
!WRF3.9fr1 call get_zu_zd_pdf_fim(rand_vmas(i),zubeg,ipr,xland(i),zuh2,"UP",ierr(i),k22(i), &
!WRF3.9fr1  kfinalzu,zuo(i,kts:kte),kts,kte,ktf,beta_u,kpbl(i),csum(i),pmin_lev(i))
           call get_zu_zd_pdf_fim(kklev,p_cup(i,:),rand_vmas(i),zubeg,ipr,xland(i),zuh2,"UP",ierr(i),k22(i), & !WRF3.9
            kfinalzu,zuo(i,kts:kte),kts,kte,ktf,beta_u,kstabi(i),csum(i),pmin_lev(i)) !WRF3.9
        endif
      endif ! end deep
      if ( name == 'mid' ) then
       if(ktop(i) <= kbcon(i)+2)then
           ...
           kfinalzu=ktop(i)
           ktopdby(i)=ktop(i)+1
!          call get_zu_zd_pdf_fim(rand_vmas(i),zubeg,ipr,xland(i),zuh2,"MID",ierr(i),k22(i),kfinalzu,zuo(i,kts:kte),kts,kte,ktf,beta_u,kbcon(i),csum(i),pmin_lev(i)) !WRF3.9 fr1
...
!           kbegin=0
!           dzh2=(z_cup(i,kfinalzu)-z_cup(i,k22(i)))/40._r8
!           zh2(1)=z_cup(i,k22(i))
!           if(zuh2(1).gt.0.1_r8 .and. kbegin.eq.0)kbegin=1
!           do k=2,40
!             zh2(k)=zh2(k-1)+dzh2
!             if(zuh2(k).gt.0.1_r8 .and. kbegin.eq.0)kbegin=k
!           enddo
!           zuo(i,k22(i))=zuh2(kbegin)
...
!-------------------------------------------------------------------------
! SUBROUTINE get_zu_zd_pdf_fim(rand_vmas,zubeg,ipr,xland,zuh2,draft,ierr,kb,kt,zu,kts,kte,ktf,max_mass,kpbli,csum,pmin_lev) !WRF3.9 fr1
 SUBROUTINE get_zu_zd_pdf_fim(kklev,p,rand_vmas,zubeg,ipr,xland,zuh2,draft,ierr,kb,kt,zu,kts,kte,ktf,max_mass,kpbli,csum,pmin_lev) !WRF3.9

 implicit none
! integer, intent(in) ::ipr,xland,kb,kt,kts,kte,ktf,kpbli,csum,pmin_lev      !WRF3.9 fr1
 integer, intent(in)  ::ipr,xland,kb,kklev,kt,kts,kte,ktf,kpbli,csum,pmin_lev !WRf3.9
 real(r8), intent(in) ::max_mass,zubeg
 real(r8), intent(inout) :: zu(kts:kte)
 real(r8), intent(in) :: p(kts:kte) !WRF3.9
 real(r8)  :: zuh(kts:kte),zuh2(1:40)
 integer, intent(inout) :: ierr
 character*(*), intent(in) ::draft

 !- local var
 integer :: kk,k,kb_adj,kpbli_adj
 real(r8)    :: krmax,beta, alpha,kratio,tunning,FZU,rand_vmas
 real(r8)    :: lev_start !WRF3.9
 ...
 zuh=0.0_r8
   kb_adj=max(kb,2)
 IF(draft == "UP") then
   lev_start=min(.9_r8,.4_r8+csum*.013_r8) !WRF3.9
   kb_adj=max(kb,2)

! WRF3.9..
   tunning=p(kt)+(p(kpbli)-p(kt))*lev_start
   tunning =min(0.9_r8, (tunning-p(kb_adj))/(p(kt)-p(kb_adj))) !=.6
   tunning =max(0.2_r8, tunning)
   beta    = 1.3_r8 !2.5 ! max(2.5,2./tunning)
   alpha= (tunning*(beta -2._r8)+1._r8)/(1._r8-tunning)
...
!#endif
   do k=kb_adj,min(kte,kt)
      ...
      zu(k) = zubeg+FZU*kratio**(alpha-1.0_r8) * (1.0_r8-kratio)**(beta-1.0_r8)
   enddo
!.. WRF3.9
...
!     tunning = 2._r8-min(1._r8,float(csum)*.03_r8)
!     if(rand_vmas /= 0._r8 ) tunning=1.5_r8+.5_r8*rand_vmas
!     beta    = max(1.5_r8,2._r8/tunning)
!!
!! make profile sharper when higher up or lower (compared to middle)
!!
!  alpha   = tunning*beta+abs(1.5_r8-tunning)*5._r8
!  fzu=1._r8
...
!       tunning=tunning+.1_r8
!       beta    = max(1.5_r8,2._r8/tunning)
!!       alpha   = tunning*beta
!       alpha   = tunning*beta+abs(1.5_r8-tunning)*5._r8
!       if(tunning > 2.1_r8) ierr=123
!       if(tunning > 2.1_r8) exit
!       do k=kb_adj,min(kte,kt+1)
!           kratio= float(k)/float(kt+1)
!           zu(k) = zubeg+FZU*kratio**(alpha-1.0_r8) * (1.0_r8-kratio)**(beta-1.0_r8)
!        enddo
...
! WRF3.9..
 ELSEIF(draft == "SH2") then
   tunning =min(0.8_r8, (p(kpbli)-p(kb_adj))/(p(kt)-p(kb_adj))) !=.6
   tunning =max(0.2_r8, tunning)
   beta    = 2.5_r8 !2.5 ! max(2.5,2./tunning)
   alpha= (tunning*(beta -2._r8)+1._r8)/(1._r8-tunning)
!#if ( ! defined NO_GAMMA_SUPPORT )
   fzu = gamma(alpha + beta)/(gamma(alpha)*gamma(beta))
!#endif
...
!.. WRF3.9
 ...
 ELSEIF(draft == "SH3") then
  tunning = 0.6_r8
!  beta    =2.2_r8/tunning !WRF3.9 fr1
!  alpha   = tunning*beta  !WRF3.9 fr1
   beta    = 3.5 ! max(2.5,2./tunning)  !WRF3.9
   alpha   = beta -2. ! +1 !max(1.1,tunning*beta-abs(1.5-tunning)*5.)  !WRF3.9
  fzu=1._r8
  do k=1,40
      kratio= float(k)/float(40)
      zuh2(k) = zubeg+FZU*kratio**(alpha-1.0_r8) * (1.0_r8-kratio)**(beta-1.0_r8)
   enddo
   if(maxval(zuh2(1:40)).gt.0._r8)  &
      zuh2(1:40)= zuh2(1:40)/ maxval(zuh2(1:40))
...
!! too low
!   do while (maxloc(zu(:),1) < pmin_lev)
!       tunning=tunning+.05_r8
!       if(tunning > 1._r8) ierr=123
!       if(tunning > 1._r8) exit
!       beta    = max(2.5_r8,2._r8/tunning)
!       alpha   = tunning*beta
!       do k=kb_adj,min(kte,kt+1)
!           kratio= float(k)/float(kt+1)
!           zu(k) = zubeg+FZU*kratio**(alpha-1.0_r8) * (1.0_r8-kratio)**(beta-1.0_r8)
...
!.. WRF3.9 fr1
 ...
 ELSEIF(draft == "DOWN" .or. draft == "DOWNM") then
  ...
  ! tunning = 0.8_r8
  ! beta    = 3.0_r8/tunning
!  tunning = 0.8_r8          !WRF3.9 fr1
!  beta    =2.0_r8/tunning   !WRF3.9 fr1
!  alpha   = tunning*beta    !WRF3.9 fr1
!  fzu=1._r8                 !WRF3.9 fr1
!  zuh(:)=0._r8              !WRF3.9 fr1
...
!.. WRF3.9
  ...
  zu(:)=0._r8
  !do k=kts,min(kt+1,ktf) !WRF3.9 fr1
  !    kratio= float(k)/float(kt+1) !WRf3.9 fr1
  !    zuh(k) = FZU*kratio**(alpha-1.0_r8) * (1.0_r8-kratio)**(beta-1.0_r8)
  !enddo
  do k=2,min(kt,ktf)    !WRF3.9
      kratio= (p(k)-p(1))/(p(kt)-p(1)) !WRF3.9
      zu(k) = FZU*kratio**(alpha-1.0_r8) * (1.0_r8-kratio)**(beta-1.0_r8)
   enddo
...
!    if(zu(2).gt.max_mass)fzu=max_mass/zu(2) ! max(0.,zu(2)-max_mass) !WRF3.9 fr1
!     do k=2,kt+1       !WRF3.9 fr1
!       zu(k)=fzu*zu(k) !WRF3.9 fr1
!     enddo             !WRF3.9 fr1
     zu(1)=0._r8
...
!
        ...
        DO 100 i=its,itf
         ...
         dA=  DZ*9.81_r8*( tn(i,k)-t(i,k) + 0.608_r8*(qo(i,k)-q(i,k)))/dtime
         AA0(I)=AA0(I)+dA
100     CONTINUE
...
!----------------------------------------------------------------------
 SUBROUTINE get_inversion_layers(ierr,p_cup,t_cup,z_cup,qo_cup,qeso_cup,k_inv_layers,&
        ...
        integer, dimension (pcols) ,intent (in ) :: ierr,kstart,kend
        integer, dimension (pcols) :: kend_p3

        real(r8),    dimension (pcols,kts:kte), intent (in ) :: p_cup,t_cup,z_cup,qo_cup,qeso_cup
        real(r8),    dimension (pcols,kts:kte), intent (out) :: dtempdz
        integer, dimension (pcols,kts:kte), intent (out) :: k_inv_layers
        !-local vars
        real(r8)   :: dp,l_mid,l_shal,first_deriv(kts:kte),sec_deriv(kts:kte)
        integer:: ken,kadd,kj,i,k,ilev,kk,ix,k800,k550,mid,shal
        !
        !-initialize k_inv_layers as undef
        !l_mid=400._r8 !WRF3.9 fr1
        l_mid=300._r8  !WRF3.9
        l_shal=100._r8
        k_inv_layers(:itf,:) = 1
         do i = its,itf
           if(ierr(i) == 0)then
...
!-----------------------------------------------------------------------------------
 FUNCTION DERIV3(xx, xi, yi, ni, m)
    ...
    if (m == 1) then
        deriv3 =          (2.0_r8*xx - (x(2)+x(3)))*f(1)/((x(1)-x(2))*(x(1)-x(3)))
        deriv3 = deriv3 + (2.0_r8*xx - (x(1)+x(3)))*f(2)/((x(2)-x(1))*(x(2)-x(3)))
        deriv3 = deriv3 + (2.0_r8*xx - (x(1)+x(2)))*f(3)/((x(3)-x(1))*(x(3)-x(2)))
    ! calculate the second-order derivative using Lagrange interpolation
      else
        deriv3 =          2.0_r8*f(1)/((x(1)-x(2))*(x(1)-x(3)))
        deriv3 = deriv3 + 2.0_r8*f(2)/((x(2)-x(1))*(x(2)-x(3)))
        deriv3 = deriv3 + 2.0_r8*f(3)/((x(3)-x(1))*(x(3)-x(2)))
    end if
 END FUNCTION DERIV3
!=============================================================================================
  SUBROUTINE get_lateral_massflux(itf,ktf, its,ite, kts,kte       &
     ...
     character *(*), intent (in) :: draft
     integer, intent(in):: itf,ktf, its,ite, kts,kte
     integer, intent(in)   , dimension(pcols) :: ierr,ktop,kbcon,k22
     real(r8),    intent(in),  OPTIONAL , dimension(pcols):: lambau
     real(r8),    intent(in)   , dimension(pcols,kts:kte) :: zo_cup,zuo
     real(r8),    intent(inout), dimension(pcols,kts:kte) :: cd,entr_rate_2d
     real(r8),    intent(  out), dimension(pcols,kts:kte) :: up_massentro, up_massdetro&
                                                            ,up_massentr,  up_massdetr
     real(r8),    intent(  out), dimension(pcols,kts:kte),  OPTIONAL :: &
                                                             up_massentru, up_massdetru
     !-- local vars
     Integer :: i,k, incr1,incr2
     real(r8) :: dz,trash,trash2
     ...
     if(present(up_massentru) .and. present(up_massdetru))then
       do k=kts,kte
        do i=its,ite
          up_massentru(i,k)=0._r8
          up_massdetru(i,k)=0._r8
        enddo
       enddo
     endif
     DO i=its,itf
       if(ierr(i).eq.0)then
          ...
          do k=max(2,k22(i)+1),maxloc(zuo(i,:),1)
           ...
           if(up_massentro(i,k-1).lt.0._r8)then
              up_massentro(i,k-1)=0._r8
              up_massdetro(i,k-1)=zuo(i,k-1)-zuo(i,k)
              if(zuo(i,k-1).gt.0._r8)cd(i,k-1)=up_massdetro(i,k-1)/(dz*zuo(i,k-1))
           endif
           if(zuo(i,k-1).gt.0._r8)entr_rate_2d(i,k-1)=(up_massentro(i,k-1))/(dz*zuo(i,k-1))
         enddo
         do k=maxloc(zuo(i,:),1)+1,ktop(i)
           ...
           if(up_massdetro(i,k-1).lt.0._r8)then
              up_massdetro(i,k-1)=0._r8
...
!   IF(.not.restart)THEN
     ...
     DO k=kts,kte
     DO i=its,ite
        RTHCUTEN(i,k,j)=0._r8
        RQVCUTEN(i,k,j)=0._r8
        RUCUTEN(i,k,j)=0._r8
        RVCUTEN(i,k,j)=0._r8
     ENDDO
     ENDDO
