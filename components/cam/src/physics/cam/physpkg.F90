...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
  ...
  subroutine tphysac (ztodt,   cam_in,  &
    ...
    use cam_diagnostics,    only: diag_phys_tend_writeout, calc_tot_energy
...
!++CMZ
    if ( dycore_is('LR') .or. dycore_is('MPAS') ) call set_dry_to_wet(state)    ! Physics had dry, dynamics wants moist
!--CMZ
...
!--BEH
    ...
    call calc_tot_energy(state, 'pBF')
    if (dycore_is('LR') .or. dycore_is('SE').or.dycore_is('MPAS'))  then
       call check_energy_fix(state, ptend, nstep, flx_heat)
       call physics_update(state, ptend, ztodt, tend)
       call check_energy_chng(state, tend, "chkengyfix", nstep, ztodt, zero, zero, zero, flx_heat)
       call outfld( 'EFIX', flx_heat    , pcols, lchnk   )
    end if
    call calc_tot_energy(state, 'pBP')
