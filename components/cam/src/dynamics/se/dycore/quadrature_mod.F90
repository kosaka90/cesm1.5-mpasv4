...
contains
  ...
  function gauss_pts(npts) result(pts)
    ...
    pts(1) = -0.93246951420315202781_real_kind
    pts(2) = -0.66120938646626451366_real_kind
    pts(3) = -0.23861918608319690863_real_kind
    pts(4) = -pts(3)
    pts(5) = -pts(2)
    pts(6) = -pts(1)
  ...
  function gauss_wts(npts,pts) result(wts)
    ...
    wts(1)  =  0.17132449237917034504_real_kind
    wts(2)  =  0.36076157304813860756_real_kind
    wts(3)  =  0.46791393457269104738_real_kind
    wts(4)  =  wts(3)
    wts(5)  =  wts(2)
    wts(6)  =  wts(1)
  ...
  subroutine trapN(f,a,b,N,it,s)
    ...
    if (N==1) then
       s = 0.5_real_kind*(b-a)*(f(a) + f(b))
       it =1
    else
       ssum = 0.0_real_kind
       rtnm =1.0_real_kind/it
       del = (b-a)*rtnm
       x=a+0.5_real_kind*del
       do j=1,it
          ssum = ssum + f(x)
          x=x+del
       end do
       s=0.5_real_kind*(s + del*ssum)
       it=2*it  
    end if
  ...
  function simpsons(f,a,b,eps) result(Integral)
    ...
    do while ((N<=Nmax .and. ABS(s-os)>eps*ABS(os) ) .or. N<=2)
       ...
       s=(4.0_real_kind*st-ost)/3.0_real_kind
#ifdef _QUAD_DBG
  ...
  function gaussian_int(f,a,b,gs) result(Integral)
    ...
    do i=1,SIZE(gs%points)
       x = 0.50_real_kind*((b-a)*gs%points(i) + (b+a))
       s = s + gs%weights(i)*f(x)
    end do
    Integral = s*(0.5_real_kind*(b-a))
