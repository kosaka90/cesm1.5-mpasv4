...
!DIR$ UNROLL(NP)
  ...
  subroutine preq_filter(elem, edge3p1, flt, hybrid, nfilt, nets, nete)
    ...
    if (flt%type == "bv") then
       do ie=nets,nete
          elem(ie)%state%ps(:,:,nfilt) = EXP(elem(ie)%state%lnps(:,:,nfilt))
          call prim_filt_bv(elem(ie)%state%ps(:,:,nfilt),elem(ie)%state%T(:,:,:,nfilt), elem(ie)%state%v(:,:,:,:,nfilt), flt)

          do j=1,np
             do i=1,np
                elem(ie)%state%ps(i,j,nfilt) = elem(ie)%mp(i,j)*elem(ie)%state%ps(i,j,nfilt)
             end do
          end do
          do k=1,nlev
             do j=1,np
          ...
          kptr=3*nlev
          call edgeVpack(edge3p1,elem(ie)%state%ps(:,:,nfilt),1,kptr,ie)
       ...
       call bndry_exchangeV(hybrid,edge3p1,location='preq_filter')
       ...
       do ie=nets,nete
          ...
          kptr=3*nlev
          call edgeVunpack(edge3p1, elem(ie)%state%ps(:,:,nfilt), 1, kptr, ie)

          do j=1,np
             do i=1,np
                elem(ie)%state%ps(i,j,nfilt) = elem(ie)%rmp(i,j)*elem(ie)%state%ps(i,j,nfilt)
             end do
          end do
          elem(ie)%state%lnps(:,:,nfilt) = LOG(elem(ie)%state%ps(:,:,nfilt))
...
!$OMP BARRIER
       ...
       do ie=nets,nete
          elem(ie)%state%ps(:,:,nfilt) = EXP(elem(ie)%state%lnps(:,:,nfilt))
          call prim_filt_fm(elem(ie)%state%ps(:,:,nfilt),          &
...
!$omp parallel do private(k,i,j)
       ...
       call bndry_exchangeV(hybrid,edge_3lp1,location='prim_filter')
