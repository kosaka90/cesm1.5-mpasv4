...
subroutine remap1(Qdp,nx,qstart,qstop,qsize,dp1,dp2,hybrid)
  ...
  real (kind=real_kind)  :: f_xm,level1,level2,level3,level4,level5, &
                            ...
                            zero = 0._real_kind,one = 1._real_kind,tiny = 1.e-12_real_kind,qmax = 1.e50_real_kind
  ...
  integer(kind=int_kind) :: qbeg, qend
  logical :: abort=.false.
...
#ifdef CAM
!  write(6,*) 'YOU ARE DOINGS SPLINES REMAP WHICH IS NOT STRICTLY MONOTONE! ABORT' !add phl
!  abort = .true.                                                                  !add phl
#endif
  do q=qstart,qstop
  do i=1,nx
    do j=1,nx
      ...
      if (ABS(z2c(nlev+1)-z1c(nlev+1)).GE.0.000001_real_kind) then
      ...
      zgam(1)      = 0.0_real_kind
      zgam(nlev+1) = 1.0_real_kind
      ...
      do k=1,nlev
        ...
        if (filter_code(k) > 0) then
          ...
          level3 = 0.5_real_kind*(rhs(k)+h(k))
...
!$omp    private(q,i,j,z1c,z2c,zv,k,dp_np1,dp_star,Qcol,zkr,ilev) &
...
subroutine remap_Q_ppm(Qdp,nx,qstart,qstop,qsize,dp1,dp2)
  ...
  do j = 1 , nx
    do i = 1 , nx
      ...
      pio(nlev+2) = pio(nlev+1) + 1._real_kind  !This is here to allow an entire block of k threads to run in the remapping phase.
      ...
      do k = 1 , nlev
        ...
        z2(k) = ( pin(k+1) - ( pio(kk) + pio(kk+1) ) * 0.5_real_kind ) / dpo(kk)  !PPM interpolants are normalized to an independent
...
function compute_ppm_grids( dx )   result(rslt)
  ...
  do j = indB , indE
    ...
    rslt( 2,j) = ( 2._real_kind*dx(j-1) + dx(j) ) / ( dx(j+1) + dx(j) )
    rslt( 3,j) = ( dx(j) + 2._real_kind*dx(j+1) ) / ( dx(j-1) + dx(j) )
  enddo
  ...
  do j = indB , indE
    ...
    rslt( 5,j) = 1._real_kind / sum( dx(j-1:j+2) )
    rslt( 6,j) = ( 2._real_kind * dx(j+1) * dx(j) ) / ( dx(j) + dx(j+1 ) )
    rslt( 7,j) = ( dx(j-1) + dx(j  ) ) / ( 2._real_kind * dx(j  ) + dx(j+1) )
    rslt( 8,j) = ( dx(j+2) + dx(j+1) ) / ( 2._real_kind * dx(j+1) + dx(j  ) )
    rslt( 9,j) = dx(j  ) * ( dx(j-1) + dx(j  ) ) / ( 2._real_kind*dx(j  ) +    dx(j+1) )
    rslt(10,j) = dx(j+1) * ( dx(j+1) + dx(j+2) ) / (    dx(j  ) + 2._real_kind*dx(j+1) )
  enddo
...
function compute_ppm( a , dx )    result(coefs)
  ...
  do j = indB , indE
    ...
    dma(j) = minval( (/ abs(da) , 2._real_kind * abs( a(j) - a(j-1) ) , 2._real_kind * abs( a(j+1) - a(j) ) /) ) * sign(1.D0,da)
    if ( ( a(j+1) - a(j) ) * ( a(j) - a(j-1) ) <= 0._real_kind ) dma(j) = 0._real_kind
  enddo
  ...
  do j = indB , indE
    al = ai(j-1)
    ar = ai(j  )
    if ( (ar - a(j)) * (a(j) - al) <= 0._real_kind ) then
      al = a(j)
      ar = a(j)
    endif
    if ( (ar - al) * (a(j) - (al + ar)/2._real_kind) >  (ar - al)**2/6._real_kind ) al = 3._real_kind*a(j) - 2._real_kind * ar
    if ( (ar - al) * (a(j) - (al + ar)/2._real_kind) < -(ar - al)**2/6._real_kind ) ar = 3._real_kind*a(j) - 2._real_kind * al
    !Computed these coefficients from the edge values and cell mean in Maple. Assumes normalized coordinates: xi=(x-x0)/dx
    coefs(0,j) = 1.5_real_kind * a(j) - ( al + ar ) / 4._real_kind
    coefs(1,j) = ar - al
    coefs(2,j) = -6._real_kind * a(j) + 3._real_kind * ( al + ar )
  enddo
...
!           call remap_UV_ref2lagrange(np1,dt,elem,hvcoord,ie)
          do k=1,nlev
             dp(:,:,k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
                  ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps(:,:,np1)
...
!    call t_stopf ('euler_step')
    ...
    call qdp_time_avg( elem , rkstage , n0_qdp , np1_qdp , limiter_option , nu_p , hybrid, nets , nete )
...
!-----------------------------------------------------------------------------
  ...
  subroutine qdp_time_avg( elem , rkstage , n0_qdp , np1_qdp , limiter_option , nu_p , hybrid , nets , nete )
      ...
      do q=qbeg,qend
        do k=kbeg,kend
          elem(ie)%state%Qdp(:,:,k,q,np1_qdp) =               &
                   ( elem(ie)%state%Qdp(:,:,k,q,n0_qdp) + &
                     (rkstage-1)*elem(ie)%state%Qdp(:,:,k,q,np1_qdp) ) / rkstage
        enddo
      enddo
    enddo
#else
...
! nelemd
  ...
  do ie=nets,nete
     ...
     if(DSSopt==DSSno_var)then
        do q=qbeg,qend
          kptr = nlev*(q-1) + kbeg - 1
          call edgeVpack(edgeAdv, elem(ie)%state%Qdp(:,:,kbeg:kend,q,n0_qdp), kblk , kptr , ie )
        enddo
     else
        do q=qbeg,qend
          kptr = nlev*(q-1) + kbeg - 1
          call edgeVpack(edgeAdvp1, elem(ie)%state%Qdp(:,:,kbeg:kend,q,n0_qdp),kblk , kptr , ie )
        enddo
        ...
        do k=kbeg,kend
           ...
           if (npdg<np) then
              ...
              do i=1,npdg
                 ...
                 if ( i==npdg) pshat(i,j)=pshat(i,j)*.90_real_kind
                 if ( j==npdg) pshat(i,j)=pshat(i,j)*.90_real_kind
              enddo
              enddo
...
!JMD!$OMP BARRIER
  ...
  ! output:
  !     state%ps(np1)          surface pressure at time np1
  ...
  use parallel_mod, only : abortmp
  use hybrid_mod     , only : hybrid_t
  use hybrid_mod    , only : set_region_num_threads, get_loop_ranges
  use derivative_mod, only : interpolate_gll2fvm_points
  use fvm_control_volume_mod, only : fvm_struct
  use control_mod, only : se_prescribed_wind_2d
  use dimensions_mod        , only : qsize_condensate_loading, ldry_mass_vertical_coordinates, ntrac
  use hybrid_mod , only : get_loop_ranges
  ...
  integer :: q
  real (kind=real_kind), dimension(np,np,nlev)  :: dp_moist,dp_star_moist, dp_inv,dp_dry,dp_star_dry
...
!     elem(ie)%derived%eta_dot_dpdn(:,:,:)=0
     if (rsplit==0) then
        ...
        do k=1,nlev
           dp_moist(:,:,k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
                ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps(:,:,np1)
           dp_star_moist(:,:,k) = dp_moist(:,:,k) + dt*(elem(ie)%derived%eta_dot_dpdn(:,:,k+1) -&
                elem(ie)%derived%eta_dot_dpdn(:,:,k))
        enddo
        dp_inv=1.0D0/dp_moist !for efficiency
        !
        ! remap tracers
        !
        if (minval(dp_star_moist)<0) call abortmp('negative layer thickness.  timestep or remap time too large A')
        call remap1(elem(ie)%state%Qdp(:,:,:,:,np1_qdp),np,1,qsize,qsize,dp_star_moist,dp_moist,hybrid=hybrid)
      else

        !  REMAP u,v,T from levels in dp3d() to REF levels
        !
        ! update final ps
        elem(ie)%state%ps(:,:,np1) = hvcoord%hyai(1)*hvcoord%ps0 + &
        ...
        if (ldry_mass_vertical_coordinates) then
          ...
          enddo
          if (minval(dp_star_dry)<0) call abortmp('negative dry layer thickness.  timestep or remap time too large B')
          !
          dp_star_moist(:,:,:) = dp_star_dry(:,:,:)
          do q=1,qsize_condensate_loading
            do k=1,nlev
              dp_star_moist(:,:,k)= dp_star_moist(:,:,k)+elem(ie)%state%Qdp(:,:,k,q,np1_qdp)
              !
              ! for some reason the outcommented line leads to incorrect results?
              !
              !            dp_star_moist(:,:,k) = SUM(elem(ie)%state%Qdp(:,:,k,1:qsize_condensate_loading,np1_qdp))+dp_star_dry(:,:,k)
            end do
          end do
          if (minval(dp_star_moist)<0) call abortmp('negative moist layer thickness.  timestep or remap time too large')

          call remap1(elem(ie)%state%Qdp(:,:,:,:,np1_qdp),np,1,qsize,qsize,dp_star_dry,dp_dry,hybrid=hybrid)
          !
          ...
          do q=1,qsize_condensate_loading
            do k=1,nlev
              !            dp_moist(:,:,k) = SUM(elem(ie)%state%Qdp(:,:,k,1:qsize_condensate_loading,np1_qdp))+dp_dry(:,:,k)
              dp_moist(:,:,k) = dp_moist(:,:,k)+elem(ie)%state%Qdp(:,:,k,q,np1_qdp)
            end do
          end do
          if (minval(dp_star_moist)<0) call abortmp('negative layer thickness.  timestep or remap time too large')
        else
          do k=1,nlev
            dp_moist(:,:,k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
                 ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps(:,:,np1)
            dp_star_moist(:,:,k) = elem(ie)%state%dp3d(:,:,k,np1)
          enddo
          if (minval(dp_star_moist)<0) call abortmp('negative layer thickness.  timestep or remap time too large')
          call t_startf('remap1_t')
          call remap1(elem(ie)%state%Qdp(:,:,:,:,np1_qdp),np,1,qsize,qsize,dp_star_moist,dp_moist,hybrid=hybrid)
          call t_stopf ('remap1_t')
        end if
