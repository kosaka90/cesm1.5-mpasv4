...
contains
  ...
  subroutine initEdgeSBuffer(par,edge,elem,nlyr,bptr,rptr,bndry_type, nthreads)
    implicit none 
    ...
    call initEdgeBuffer(par,edge,elem,nlyr,NewMethod=nMethod,bndry_type=bndry_type, &
                         nthreads=nthreads)
...
!IDEA   subroutine initEdgeBuffer(par,edge,elem,nlyr,buf_ptr, receive_ptr, NewMethod)
  subroutine initEdgeBuffer(par,edge,elem,nlyr, NewMethod,bndry_type,nthreads)
...
!    print *,'IAM: ',iam,'initedgebuffer: nSendCycles: ', nSendCycles 
!    print *,'IAM: ',iam,'initedgebuffer: nRecvCycles: ', nRecvCycles 

    allocate(edge%receive(nbuf))   
    allocate(edge%buf(nbuf))

    if(edge%bndry_type == HME_BNDRY_A2A) then 
       allocate(edge%scounts(nSendCycles),edge%sdispls(nSendCycles))
       allocate(edge%rcounts(nRecvCycles),edge%rdispls(nRecvCycles))

       if(present(NewMethod)) then 
          do icycle=1,nRecvCycles
             pCycle               => pSchedule%RecvCycle(icycle)
             edge%rcounts(icycle) =  nlyr * pCYcle%lengthS
             edge%rdispls(icycle) =  nlyr * (pCycle%ptrS -1) 
          enddo
          do icycle=1,nSendCycles
             pCycle               => pSchedule%SendCycle(icycle)
             edge%scounts(icycle) =  nlyr * pCYcle%lengthS
             edge%sdispls(icycle) =  nlyr * (pCycle%ptrS -1)
          enddo
       else
          do icycle=1,nRecvCycles
             pCycle               => pSchedule%RecvCycle(icycle)
             edge%rcounts(icycle) =  nlyr * pCYcle%lengthP
             edge%rdispls(icycle) =  nlyr * (pCycle%ptrP -1)
          enddo
          do icycle=1,nSendCycles
             pCycle               => pSchedule%SendCycle(icycle)
             edge%scounts(icycle) =  nlyr * pCYcle%lengthP
             edge%sdispls(icycle) =  nlyr * (pCycle%ptrP -1)
          enddo
       endif
#if 0
       if(iam == 1) then 
          print *,'initedgeBuffer: nlyr ',nlyr
          print *,'initedgeBuffer: nSendCycles ',nSendCycles
          print *,'initedgeBuffer: nRecvCycles ',nRecvCycles
          print *,'initedgeBuffer: scounts:    ', edge%scounts
          print *,'initedgeBuffer: sdispls: ', edge%sdispls
          print *,'initedgeBuffer: rcounts: ', edge%rcounts
          print *,'initedgeBuffer: rdispls: ', edge%rdispls
       endif
#endif
    endif
