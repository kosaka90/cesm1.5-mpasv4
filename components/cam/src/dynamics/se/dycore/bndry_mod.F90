...
module bndry_mod
  use parallel_mod, only : abortmp,iam, HME_BNDRY_P2P, HME_BNDRY_MASHM, HME_BNDRY_A2A
...
contains
  ...
  subroutine bndry_exchangeV_a2a(par,nthreads,ithr,buffer,location)
    ...
    use parallel_mod, only : parallel_t, abortmp, status, srequest, rrequest, &
         mpireal_t, mpiinteger_t, mpi_success, mpi_version
#else
    ...
    character(len=*),       parameter :: subname = 'bndry_exchangeV_a2a'
...
!      call t_startf('bndry_a2a')
      call MPI_neighbor_Alltoallv(buffer%buf,buffer%scounts,buffer%sdispls,MPIreal_t, &
                     buffer%receive,buffer%rcounts,buffer%rdispls,MPIreal_t,par%comm_dist_graph,ierr)
...
!      call t_stopf('bndry_a2a')
      if ( lenMoveptr .ne. nthreads) then
        ...
        write(*,30) 'bndry_exchangeV_a2a',location,lenMoveptr,nthreads
      endif
   else
      if ( lenMoveptr .ne. nthreads) then
        ompthreadMissmatch = .true.
...
#else
    call abortmp('bndry_exchangeV_a2a requires MPI-3 feature support')
#endif

30  format(a,'Potential performance issue: ',a,'LenMoveptr,nthreads: ',2(i3))

  end subroutine bndry_exchangeV_a2a
...
!    call t_startf('bndry_p2p')
!    call t_startf('bndry_irecv')
    !==================================================
    !  Post the Receives
    !==================================================
    do icycle=1,nRecvCycles
       ...
       if(Debug) print *,subname,': MPI_Irecv: SRC:',source,'LENGTH:',length,'TAG: ',tag, 'PTR: ',iptr
...
!    call t_stopf('bndry_irecv')

!    call t_startf('bndry_isend')
    !==================================================
    !  Fire off the sends
    !==================================================
    do icycle=1,nSendCycles
       ...
       tag             = buffer%tag
       iptr            = nlyr * (pCycle%ptrP - 1) + 1
       if(Debug) print *,subname,': MPI_Isend: DEST:',dest,'LENGTH:',length,'TAG: ',tag, 'PTR: ',iptr
...
!    call t_stopf('bndry_isend')

      if ( lenMoveptr.ne.nthreads ) then
         ompthreadMissmatch = .true.
         write(*,30) 'bndry_exchangeV_p2p',location,lenMoveptr,nthreads
      endif

!      call t_startf('bndry_waitall')
...
!      call t_stopf('bndry_waitall')
  else  ! else for non-master threads
      ! If not the master thread test for a missmatch in the assumed number of
      ! OpenMP threads and the actual
      if ( lenMoveptr.ne. nthreads) then
         ompthreadMissmatch = .true.
         write(*,30) 'bndry_exchangeV_p2p',location,lenMoveptr,nthreads
      endif
...
!    call t_startf('bndry_copy')
    ...
    if (.not. ompthreadMissmatch) then
       ...
       ! Have the master thread do all the copying if there is a missmatch in
       ! the assumed versus actual number of OpenMP threads
       do j=1,lenMovePtr
          iptr   = buffer%moveptr(j)
          length = buffer%moveLength(j)
          if(length>0) then
...
30  format(a,' Potential performance issue: ',a,'LenMoveptr,nthreads: ',2(i3))
  ...
  if(ithr == 0) then
    ...
    ! Test for a miss match in the assumed versus actual
    ! number of OpenMP threads
    if ( lenMovePtr .ne. nthreads) then
       ompthreadMissmatch = .TRUE.
       write(*,30) subname,location,lenMoveptr,nthreads
    endif
    ...
    if (.not. ompthreadMissmatch) then
       ...
       ! Have the master thread do all the copying if there is a
       ! missmatch in
       ! the assumed versus actual number of OpenMP threads
       do j=1,lenMovePtr
          iptr   = buffer%moveptr(j)
...
30  format(a,'Potential performance issue: ',a,'LenMoveptr,nthreads: ',2(i3))
  ...
  if(ithr == 0) then
    if ( lenMovePtr .ne. nthreads ) then
       ompthreadMissmatch = .true.
       print *,'Potential performance issue:', location, ': lenMoveptr, nthreads: ',lenMoveptr, nthreads
    endif
...
!    call t_startf(subname)
    ...
    if(buffer%bndry_type == HME_BNDRY_A2A) then
       call bndry_exchangeV_a2a(hybrid%par,hybrid%nthreads,hybrid%ithr,buffer,location)
    else
...
!    call t_startf(subname)
    ...
    nthreads = 1
    if(buffer%bndry_type == HME_BNDRY_A2A) then
       call bndry_exchangeV_a2a(par,nthreads,ithr,buffer,location)
    else
...
!
  ...
  real (kind=real_kind) :: tol = 0.1_real_kind
