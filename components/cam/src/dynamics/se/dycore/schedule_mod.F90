...
contains
  ...
  subroutine genEdgeSched(par,elem, PartNumber,LSchedule,MetaVertex)
    ...
    integer, allocatable :: src(:),dest(:),srcweight(:),destweight(:)
    logical :: reorder
   ...
   is=1
    ...
    do j=1,ncycle
       ...
       if((MetaVertex%edges(j)%HeadVertex == PartNumber) .AND. &
          ...
          call SetCycle(elem, LSchedule,LSchedule%MoveCycle(1),MetaVertex%edges(j),HME_CYCLE_MOVE)
          ...
          call SetCycle(elem, LSchedule,LSchedule%SendCycle(is),MetaVertex%edges(j),HME_CYCLE_SEND)
          ...
          call SetCycle(elem, LSchedule,LSchedule%RecvCycle(ir),MetaVertex%edges(j),HME_CYCLE_RECV)
    ...
    allocate(src(nRecv),srcWeight(nRecv))
    allocate(dest(nSend),destWeight(nSend))

    do icycle=1,nSend
       dest(icycle)       = LSchedule%SendCycle(icycle)%dest - 1
       destWeight(icycle) = LSchedule%SendCycle(icycle)%lengthP
    enddo
    do icycle=1,nRecv
       src(icycle)        = LSchedule%RecvCycle(icycle)%source - 1
       srcWeight(icycle)  = LSchedule%RecvCycle(icycle)%lengthP
    enddo

    reorder=.FALSE.
#if 0
    if(iam == 1) then
       print *,'genEdgeSched: nSend: ',nSend
       print *,'genEdgeSched: nRecv: ',nRecv
       print *,'genEdgeSched: src: ',src
       print *,'genEdgeSched: srcWeight: ',srcWeight
       print *,'genEdgeSched: dest: ',dest
       print *,'genEdgeSched: destWeight: ',destWeight
    endif
#endif
    call MPI_Dist_graph_create_adjacent(par%comm, nRecv,src,srcWeight, &
                       nSend,dest,destWeight,MPI_INFO_NULL,reorder,par%comm_dist_graph,ierr)
    if(ierr .ne. MPI_SUCCESS) then
          errorcode=ierr
          call MPI_Error_String(errorcode,errorstring,errorlen,ierr)
          print *,subname,': Error after call to MPI_dist_graph_create_adjacent ',errorstring
    endif


!    print *,'genEdgeSched: after call to MPI_Dist_graph_create_adjacent'
    deallocate(src,srcWeight,dest,destWeight)
#endif
#ifdef DEBUGPART
...
97  format(5x,'METAEDGE #',I2,2x,'TYPE ',I1,2x,'G.EDGES',I4,2x,'WORDS ',I5,2x, &
  ...
  subroutine SetCycle(elem, schedule,Cycle,Edge,ctype)
    use element_mod, only : element_t
    ...
    do i=1,Edge%nmembers
       if((ctype == HME_CYCLE_SEND) .or. (ctype == HME_CYCLE_MOVE)) then
       !   Setup send index
       ...
       if((ctype == HME_CYCLE_RECV) .or. (ctype == HME_CYCLE_MOVE)) then
       !   Setup receive index
