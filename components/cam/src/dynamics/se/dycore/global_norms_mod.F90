...
!JMD    print *,'global_integral: after global_shared_sum'
  ...
  subroutine test_global_integral(elem,hybrid,nets,nete,mindxout)
    ...
    min_area = min_area*rearth*rearth/1000000._real_kind
    max_area = max_area*rearth*rearth/1000000._real_kind
    avg_area = avg_area*rearth*rearth/1000000._real_kind
...
!
    ...
    do ie=nets,nete
      elem(ie)%variable_hyperviscosity = 1.0_real_kind
    end do
...
!       viscosity in namelist specified for smallest element:
        if (fine_ne>0) then
           ...
           max_unif_dx = (111.28_real_kind*30)/dble(fine_ne)   ! in km
        endif
     ...
     if (hybrid%masterthread) then
       ...
       if (ntrac>0) then
          ...
          write(iulog,'(a,f10.2,a)') "Stability (fvm Courant number): advective (120m/s)   dt_tracer < ",&
               dble(nhe)*(2.0_real_kind*dd_pi*Rearth/dble(4.0_real_kind*ne*nc))/120.0e0_real_kind,'s'
