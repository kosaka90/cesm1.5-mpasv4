module restart_dynamics
  ...
  type(var_desc_t) :: udesc, vdesc, tdesc, psdesc, pswetdesc, phisdesc, timedesc, dp3ddesc
...
CONTAINS
  ...
  subroutine init_restart_dynamics(File, dyn_out)
    ...
    ierr = PIO_Def_Var(File, 'dp3d', pio_double, (/ncol_dimid, nlev_dimid, timelevels_dimid/), dp3ddesc)
    !
    ...
    ierr = PIO_Def_Var(File, 'PSwet', pio_double, (/ncol_dimid/), PSwetdesc)
  ...
  subroutine write_restart_dynamics(File, dyn_out)
   ...
   if (iam .lt. par%nprocs) then
    ...
    do ie=1,nelemd
       do k=1,nlev
          do j=1,np
             do i=1,np
                var3d(i,j,ie,k) = elem(ie)%derived%omega(i,j,k)
             end do
          end do
       end do
    end do
    ...
    call PIO_Setframe(File,dp3ddesc, t)
   ...
   !$omp parallel do num_threads(horz_num_threads) private(ie, j, i)
    do ie=1,nelemd
       do j=1,np
          do i=1,np
             var2d(i,j,ie) = elem(ie)%state%ps(i,j,tl)
          end do
       end do
    end do
    ...
    do ie=1,nelemd
       do j=1,np
          do i=1,np
             var2d(i,j,ie) = elem(ie)%state%pswet(i,j)
          end do
       end do
    end do
    call PIO_Write_Darray(File,PSwetdesc,iodesc2d, var2d,ierr)
...
!    integer :: ncid
    ...
    ierr = PIO_Inq_varid(File, 'PSwet', pswetdesc)
    ...
    do k=1,nlev
       do ie=1,nelemd
          do j=1,np
             do i=1,np
                cnt=cnt+1
                elem(ie)%derived%omega(i,j,k) = var3d(cnt)
             end do
          end do
       end do
    end do
    ...
    do ie=1,nelemd
       do j=1,np
          do i=1,np
             cnt=cnt+1
             elem(ie)%state%ps(i,j,tl) = var2d(cnt)
             elem(ie)%state%lnps(i,j,tl) = log(var2d(cnt))
          end do
       end do
    end do

    call pio_setframe(File,pswetdesc, int(1,kind=pio_offset_kind))
    call pio_read_darray(File, pswetdesc, iodesc2d, var2d, ierr)
    ...
    do ie=1,nelemd
       do j=1,np
          do i=1,np
             cnt=cnt+1
             elem(ie)%state%pswet(i,j) = var2d(cnt)
          end do
       end do
    end do
