...
module dp_grids
  use dimensions_mod,         only: npsq, np, fv_nphys
  ...
  character(len=80),  public :: se_write_phys_grid = "no"
  character(len=shr_kind_cl), public :: se_phys_grid_file  = "phys_grid.nc"
...
contains
  subroutine dp_init(elem,fvm)
    ...
    use dimensions_mod, only: nelem,nelemd,np,npsq,nc,irecons_tracer
    use element_mod, only: element_t
    use quadrature_mod, only: gausslobatto,quadrature_t
    use shr_kind_mod, only: r8=>shr_kind_r8
    use spmd_utils, only: masterproc, iam
    use derivative_mod, only: v2pinit
    ...
    if (fv_nphys>0) then
      ...
      if (masterproc) then
        write(iulog, *) 'dp_init: Allocated interpolation variables'
      end if

      call dp_write(elem, fvm, write_phys_grid_in=trim(se_write_phys_grid))
  ...
  subroutine dp_write(elem, fvm, write_phys_grid_in)
    use cam_abortutils,      only: endrun
    ...
    use spmd_utils,          only: mpi_integer, mpi_real8, iam
    use cam_logfile,         only: iulog
    use shr_sys_mod,         only: shr_sys_flush
    ...
    type (fvm_struct), dimension(:), intent(in) :: fvm
    character(len=*),                intent(in) :: write_phys_grid_in
    
    real(r8),parameter :: degrees = 180._r8/pi
    
    ! Local variables
    integer           :: i, ie, ierror, info, j, k, l, sendcount, status, ivtx
    integer           :: grid_corners_id, grid_rank_id, grid_size_id
    integer           :: boundsdim = -1 ! Fake dim for lev and ilev funcs
    ...
    integer           :: idlev  = -1    ! Set to no output by default
    integer           :: idlevp = -1    ! Set to no output by default
    integer           :: gridsize
    integer           :: IOrootID
    logical           :: IOroot
    ...
    !! Check to see if we are doing grid output, interp output, or neither
    if (trim(write_phys_grid_in) == "no") then
      if (masterproc) then
        write(iulog, *) 'dp_write: Not writing phys_grid file.'
      end if
      return
    else if (trim(write_phys_grid_in) /= 'SCRIP') then
      if (masterproc) then
        write(errormsg, *) 'dp_write: ERROR, bad value for write_phys_grid, ',&
             trim(write_phys_grid_in)
    ...
    ! Can't use cam_pio_createfile because it depends on dyn_grid which depends
    ! on this file. Recreate the functionality here
    !    call cam_pio_createfile(ncid, trim(se_phys_grid_file), PIO_CLOBBER)
    status = nf90_create(trim(se_phys_grid_file),nf90_clobber,ncid)
    ...
    if(masterproc) then
      write(iulog, *) 'Writing physics SCRIP grid file: ', trim(se_phys_grid_file)
    ...
    if (IOroot) then
      status = nf90_put_var(ncid, grid_dims_id, (/1/))
      if (status /= nf90_noerr) then
    ...
    do ie=1,nelemd
      lwork(:,:,ie) = degrees*fvm(ie)%center_cart_physgrid(:,:)%lat
    end do
    call mpi_gatherv(lwork, size(lwork), mpi_real8, recvbuf, recvcount,       &
         displs, mpi_real8, IOrootID, mpicom, ierror)
    if (IOroot) then
    ...
    do ie=1,nelemd
      lwork(:,:,ie) = degrees*fvm(ie)%center_cart_physgrid(:,:)%lon
    end do
    call mpi_gatherv(lwork, size(lwork), mpi_real8, recvbuf, recvcount,       &
         displs, mpi_real8, IOrootID, mpicom, ierror)
    if (IOroot) then
    ...
    do ie=1,nelemd
      do j=1,fv_nphys
        do i=1,fv_nphys
          do ivtx=1,4
            x      = fvm(ie)%vtx_cart_physgrid(i,j,1,ivtx)
            y      = fvm(ie)%vtx_cart_physgrid(i,j,2,ivtx)
            sphere = cart2spherical(x,y,elem(ie)%FaceNum)
            corners(i,j,ie,ivtx,1) = degrees * sphere%lat
            corners(i,j,ie,ivtx,2) = degrees * sphere%lon
          end do
        end do
      end do
    end do
...
!    call pio_seterrorhandling(ncid, PIO_INTERNAL_ERROR)
    ...
    call mpi_barrier(mpicom, ierror)
    if(masterproc) then
      write(iulog, *) 'Finished writing physics grid file: ', trim(se_phys_grid_file)
      call shr_sys_flush(iulog)
    end if
    call endrun('<-- NOT; dp_write: Wrote '//trim(se_phys_grid_file)//', ending run')
  ...
  subroutine fvm2phys_init(elem,fvm,fvm_nc,phys_nc,irecons,&
            ...
            weights_all_phys2fvm,weights_eul_index_all_phys2fvm,weights_lgr_index_all_phys2fvm,&
            jall_fvm2phys,jall_phys2fvm)
    use fvm_analytic_mod, only: I_00
    ...
    real (kind=real_kind), dimension(0:phys_nc+2):: xgno_phys,ygno_phys
!    real (kind=real_kind), dimension(phys_nc,phys_nc):: da_phys
    real (kind=real_kind), dimension(0:fvm_nc+2) :: xgno_fvm,ygno_fvm
!    real (kind=real_kind), dimension(fvm_nc,fvm_nc):: da_fvm


    real (kind=real_kind), dimension(ngpc):: gauss_weights, abscissae !dimension(ngauss)

    integer :: i,j
    integer, parameter :: nvertex = 4
    real (kind=real_kind), dimension(nvertex)            :: xcell,ycell

    real (kind=real_kind)   , dimension(num_weights_fvm2phys,irecons,nelemd),intent(out) :: weights_all_fvm2phys
    integer (kind=int_kind),  dimension(num_weights_fvm2phys,2,nelemd),intent(out) :: weights_eul_index_all_fvm2phys
    integer (kind=int_kind),  dimension(num_weights_fvm2phys,2,nelemd),intent(out) :: weights_lgr_index_all_fvm2phys

    real (kind=real_kind)   , dimension(num_weights_phys2fvm,irecons,nelemd),intent(out) :: weights_all_phys2fvm
    integer (kind=int_kind),  dimension(num_weights_phys2fvm,2,nelemd),intent(out) :: weights_eul_index_all_phys2fvm
    integer (kind=int_kind),  dimension(num_weights_phys2fvm,2,nelemd),intent(out) :: weights_lgr_index_all_phys2fvm

    integer                , dimension(nelemd)                       ,intent(out) :: jall_fvm2phys,jall_phys2fvm

    integer, parameter :: jmax_segments_cell = 50
    real (kind=real_kind)   , dimension(jmax_segments_cell,irecons)  :: weights_cell
    integer (kind=int_kind) , dimension(jmax_segments_cell,2)        :: weights_eul_index_cell
    integer (kind=int_kind) :: jcollect_cell,ie,h,jx,jy,jdx,jdy

    real (kind=real_kind)   :: tmp

    integer :: jx_prev,jy_prev!xxx
...
!       end do
       ...
       do j=1,phys_nc
          do i=1,phys_nc
             ...
             call compute_weights_cell(fvm_nc,nvertex,.true.,&
      ...
      jall_fvm2phys(ie)=jall_fvm2phys(ie)-1
       ...
       do j=1,fvm_nc
          do i=1,fvm_nc
             ...
             call compute_weights_cell(phys_nc,nvertex,.true.,&
