...
!JMD  type (derivative_t), allocatable   :: deriv(:)
...
! no need for a thread private data-structure
...
!JMD    hybrid = config_thread_region(par,'serial')
    ...
    call compute_frontogenesis(frontgf_thr,frontga_thr,tl,elem,deriv,hybrid,nets,nete)
    do ie=nets,nete
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
          ...
          p(:,:)   = hvcoord%hyam(k)*hvcoord%ps0 + hvcoord%hybm(k)*elem(ie)%state%pswet(:,:)
    ...
    enddo
    call bndry_exchangeV(hybrid,edge3,location='compute_frontogenesis')
    do ie=nets,nete
