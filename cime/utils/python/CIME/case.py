...
class Case(object):
    ...
    def initialize_derived_attributes(self):
        ...
        #smt_factor represents number of hyper-threading (?KSA)

        #self.cores_per_task = ((self.get_value("MAX_TASKS_PER_NODE")/smt_factor) \
        #                       / self.tasks_per_node) * 2
        #this formula does not apply for srun's -c option on NERSE machines, at least (KSA)

        self.cores_per_task = ((self.get_value("MAX_TASKS_PER_NODE")) \
                               / self.tasks_per_node)
